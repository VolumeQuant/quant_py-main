...
### 자율주행 종료 (2026-06-20)
- KRX(data.krx) 복구됨(HTTP 200). 신용스프레드 시도 → ★pykrx는 주식지수만(1001~1028), 채권/금리 yield 데이터 없음 → KR 회사채-국고채 스프레드는 ECOS(한은)/금투협 필요 → 데이터 부재로 미검증, future 과제(데이터 확보 선행).
- ★배포 검증: KRX 복구 후 calc_system_returns 실행 → 50%스케일 통합(킬스위치 on/off) 정상 작동 확인 = 섹터브레드스 50%스케일 production end-to-end 작동.
- 추가 고가치 각도 없음 → busywork 금지, 자율 루프 종료(재스케줄 안 함, 사용자 복귀상태).
- ★오늘 최종성과: 섹터브레드스 50%스케일 조기방어 발견·검증·배포(Cal 4.08→4.36, 약세MDD 24.7→19.2%) + 버그3개수정(바이오착시·주말·언어) + 임계검증(<35%/3일 KR최적) + 시장진단(협소장·삼전닉스 포물선). 전부 커밋·푸쉬·개인봇보고 완료.
