...
## v69 4팩터 재설계 (2026-03-20) — 헨리 base + 실시간/전방성

### 핵심 변경: v68 → v69

**1. z-score 방식: raw → rank-based (Blom 변환)**
- v68: PER에 invert raw z-score → skew=-2.43, V 기여도 -2%
- v69: rank → 균등분포 → 정규분포 ppf → V 기여도 20%
- 이현열(Henry) 원본 factor.ipynb와 동일 방법론

**2. 팩터 구조: 3팩터 → 4팩터 동일가중**
- v68: V30+Q35+M35 (3팩터)
- v69: V25+Q25+G25+M25 (4팩터 동일가중)
- V: PER+PBR+PCR+PSR (DY 제외 유지)
- Q: ROE+GPA+CFO (EPS개선도 제거 — 커버리지 41% 대형주 편중)
- G: 매출성장률 YoY (FnGuide 연간, 커버리지 96%) ← 신규
- M: 6M/Vol + K_ratio (2개 서브팩터) ← K_ratio 추가

**3. 섹터 중립화 범위 수정**
- v68: 전 서브팩터에 섹터 내 z-score (SK스퀘어 금융섹터 내 "비싸다" 판정 → 탈락)
- v69: V/Q/G = 전체 유니버스 rank (절대 비교), M만 섹터 내 rank (섹터 추세 제거)
- 헨리 원본도 V/Q는 전체, M만 섹터 중립

**4. 필터 체계 정리**
- 유지: ROE<0 하드게이트, MA120×0.95
- 추가: PER>200 제외 (잔존가치 없는 기업), 단일팩터 floor -1.5σ
- 제거: 과락 필터 (rank z-score 자연 방어 + floor이 대체)

**5. score_100 공식 재조정**
- v68: (ws+3)/6×100 → v69 분포에서 Top5도 72점 못 넘김
- v69: (ws+0.7)/2.4×100 → 72점↑ 평균 5.8개/일
- 진입: 74점↑ (평균 4.9개/일), 매도검토: 68점 (평균 11위)

**6. FWD_PER 보너스**
- 팩터 모델 밖에서 가산: 컨센서스 있고 EPS 개선 시 +10% std

**7. 타임아웃 전면 수정**
- krx_auth.py: pykrx session timeout=30초 (근본 원인 — 무한대기 해결)
- fnguide_crawler.py: requests timeout=10초, 컨센서스 pd.read_html timeout=10초
- gemini_analysis.py: 180000ms (밀리초 단위!)
- send_telegram_auto.py: timeout=30초

**8. 텔레그램 메시지 프레이밍**
- 이탈: "📉 순위 이탈" (사유만 표시, "매도" 미사용)
- 범례: 점수(72/68) 미노출, 행동 지시 없음
- credit_monitor: "→ 이 구간 과거 S&P 연평균" (화살표 추가)
...
- SK스퀘어: 25/25일 Top 5
- 브이엠: 25/25일 Top 20
- 한미반도체, 에이피알: floor -1.5σ로 제거
- 아이티센글로벌: PER>200 필터로 제거

### v70 TODO
- EPS개선도 보너스 방식 실험: Q_base + EPS개선도_z × α
- 분기 매출성장률 4Q avg (전년동기 데이터 축적 시)
- 아이티센글로벌 같은 "단일 외부 요인 의존" 감지 지표
- 과락 필터 완전 제거 여부 (현재 이미 제거, floor만 유지)
- 가중치 미세조정 (V 기여도 20%→25% 목표 시)

---

## v68 3팩터 체제 전환 + 섹터 중립화 (2026-03-20)

### 배경
- G팩터의 EPS개선도: FwdPER 커버리지 35%로 대형/소형 불공정
- G팩터의 매출성장률 YoY: 90일 cutoff 기준 대형주는 2024→2025, 소형주는 2023→2024 비교 (시점 불일치)
- V팩터: PER+PBR 2개만 사용 중 (이현열 원본의 PCR+PSR 제거 상태)
- z-score: 전체 유니버스 대상, 섹터 중립화 없음
- 가중치: V10+Q25+G35+M30 → 성장+모멘텀 추종으로 변질 (원래 철학 "싸게 사자" 이탈)

### 변경 사항

**1. G팩터 완전 제거**
- 매출성장률 YoY 제거 (시점 불일치 해결 불가)
- EPS개선도를 Q로 이동 (pandas mean NaN 자동 스킵 → 자연스러운 처리)
- growth_s 필드: JSON/ranking_manager/send_telegram 모두 정리

**2. V팩터 복원 (PER+PBR → PER+PBR+PCR+PSR)**
- 이현열 원본(factor.ipynb) 참고
- PCR: 현금흐름 밸류에이션 (CFO와 약한 중복이지만 분모 다름)
- PSR: 적자기업도 밸류 평가 가능 (PER이 NaN인 종목 커버)
- DY: 제거 유지 (성장 중심 전략과 충돌)
- 커버리지: PER 75%, PBR 100%, PCR 68%, PSR 98%

**3. 섹터 중립 z-score**
- 이현열 원본의 groupby('SEC_NM_KOR') + col_clean() 참고
- 전 서브팩터에 섹터별 winsorized z-score 적용 (V, Q, M 모두)
- 섹터 내 5종목 미만 시 전체 유니버스 fallback
- 15개 대형섹터(≥5), 8개 소형섹터(<5)
- 효과: 반도체 고착 완화, Top 20 섹터 10개 분산

**4. 가중치: V30 + Q35 + M35**
- V30: "싸게 사자" 원래 철학 복원 (원본 V33과 유사)
- Q35: 서브팩터 4개(ROE+GPA+CFO+EPS개선도)로 정보량 최대
- M35: 불장에서 추세 확인, 과락+ROE게이트로 M만 좋은 종목 방어
- M40 시도 → M 기여도 74%로 과도 → M35로 조정

**5. 메시지 개선**
- 이탈 섹션: "순위 이탈 — 매도 검토" + "상위 15위 밖 이탈 종목은 매도를 검토하세요"
- 선정과정: "밸류·퀄리티·모멘텀 섹터별 채점"
- 면책: 3줄→2줄 축약
- 톤: "입니다"체 통일 (AI Risk "분석했어요"→"분석했습니다")
- 범례: "매도 검토선 위 보유 유지 · 아래 또는 이탈 시 매도 검토"

### 진입/퇴출 (변경 없음)
- 진입: score_100 ≥ 72 + ✅ 3일 검증 (상위 5개 제한)
- 퇴출: score_100 < 68 (매도 검토선) + Top 20 밖 (이탈)
- 72/68 고정 상수 유지 — 섹터 중립화로 안정성 개선
- EDA 검증: 72점 이상 평균 4.3개, 매도검토선 ≈ 8~10위

### 재계산
- batch_recalculate_v68.py: 고속 재계산 (FnGuide 한 번만 로드, 날짜당 30초)
- 24일 전체 v68 기준 재계산 완료

### EDA 결과 (eda_v68_entry_exit.py)
- Top 5 턴오버: 7% (0.3개/일) — 매우 안정
- SK하이닉스/삼성전자: 24일 연속 Top 5
- 코스피 +11.8% 대비 전략 +15.2% (E≤5/X>15 기준)
- score_100 vs 순위 10위: 24일 중 17일 불일치 → score_100 유지가 적절

---

## v67 Growth 팩터 개선 실험 (2026-03-20, 전면 폐기)

### 목적
Growth 팩터의 최신성 개선 — 매출성장률을 연간 YoY에서 TTM/FY(최근4분기합 / 전년도)로 전환하여 최근 실적 변화를 빠르게 반영하려 함.

### 시도 1: 매출성장률 TTM/FY 전환 (`fnguide_crawler.py`)

**변경 내용:**
- `_extract_growth_from_cache()` 함수 신규 작성
- 공식: `sum(최근 4분기 매출) / 전년도 매출 - 1`
- MIN_BASE=20억 (기저 너무 작으면 NaN), GROWTH_CAP=±200%
- 연간 fallback: TTM 계산 불가 시 기존 연간 YoY 사용

**결과:**
- 매출성장률(TTM) 계산: 832/874개 종목 (TTM 800개 + 연간fallback 32개)
- 선익시스템(OLED 장비, 코스닥150)과 동아엘텍(선익시스템 자회사)이 상위권 진입
- 원래 연간 YoY에서는 보이지 않던 최근 분기 실적 급등이 반영됨

**문제:**
- 소형주의 작은 영업이익 기저에서 수주 1건으로 성장률 400%+ 발생
- G 극단값(+3.77~+4.72) + 주가 급등 → M 극단값 = G-M 이중 극대화
- 보성파워텍(영업이익 48억→250억, PER=104), 선익시스템(79억→370억) 등 소형 실적급등주가 Top 5 점령

### 시도 2: 영업이익성장률 도입 (`create_current_portfolio.py`, `strategy_b_multifactor.py`)

**변경 내용:**
- G 서브팩터: `EPS개선도 + 매출성장률` → `EPS개선도 + 매출성장률 + 영업이익성장률`
- TTM/FY 동일 방식으로 계산

**결과:**
- 영업이익성장률(TTM) 계산: 580/874개 종목 (TTM 564개 + 연간fallback 16개, 기저NaN 293개)
- 기저NaN이 293개나 되어 커버리지가 매우 낮음

**문제 (도입하면 안 되는 이유):**
1. 매출성장률과 높은 상관관계 — 실질적으로 같은 정보를 중복 반영
2. Q팩터(ROE/GPA/CFO)가 이미 수익성을 포착 — 이중 반영
3. 기저NaN 293개 → 전체 종목의 33%가 0(중립) 처리되어 노이즈만 증가
4. 소형주에서 기저효과(48억→250억 = +420%)가 극단적 → G 스코어 폭발

**결론:** 제거가 맞음. 절대 도입하지 말 것.

### 시도 3: PER > 80 하드필터 (`strategy_b_multifactor.py`)

**변경 내용:**
```python
# Forward PER 있으면 우선 사용, 없으면 trailing PER
effective_per = data['PER'].copy()
if 'forward_per' in data.columns:
    fwd_mask = data['forward_per'].notna() & (data['forward_per'] > 0)
    effective_per[fwd_mask] = data.loc[fwd_mask, 'forward_per']
per_high_mask = effective_per > 80
data = data[~per_high_mask].copy()
```

**결과:**
- 108개 종목 제외 (scored 390→329)
- 보성파워텍(PER=104, fwd_per 없음) 제거 성공
- 선익시스템(trailing PER=37)은 통과 — PER 자체는 높지 않음

**문제:**
- 두산에너빌리티, HD현대중공업, 셀트리온, 한화오션 등 대형 성장주도 제외됨
- 108개 종목 제거는 유니버스의 28% — 너무 공격적
- 단독 아이디어로는 유효하지만, 다른 변경과 동시 적용 시 효과 분리 불가

**향후 참고:** PER>80 하드필터는 단독으로 재검토할 가치 있음. fwd_per 우선 사용 아이디어도 좋음.

### 시도 4: ±2.5σ 카테고리 ceiling (`strategy_b_multifactor.py`)

**변경 내용:**
```python
# 재표준화 후 각 카테고리를 ±2.5σ로 클리핑
CATEGORY_CEILING = 2.5
clipped = (data[score_col].abs() > CATEGORY_CEILING).sum()
data[score_col] = data[score_col].clip(-CATEGORY_CEILING, CATEGORY_CEILING)
```

**결과:**
- 밸류 27개, 퀄리티 17개, 성장 27개, 모멘텀 25개 종목에 ceiling 적용
- G 최대값 +4.72 → +2.50, M 최대값 +3.06 → +2.50으로 제한
- 팩터 간 균형 회복 (V max +0.57, Q max +2.43, G max +2.50, M max +2.50)

**효과:**
- ceiling 없을 때 선익시스템 1위 → ceiling 적용 후 3위
- SK하이닉스, 브이엠 등 균형형 종목이 상대적으로 올라옴

**문제:**
- TTM/FY와 세트로만 필요 — 현행 연간 YoY에서는 G 분포가 좁아서 ceiling 불필요
- ceiling을 넣어도 선익시스템/동아엘텍이 여전히 Top 5 — 근본 해결 아님

### 시도 5: 기타 (시도 후 즉시 폐기)

**5-1. log(PER) 변환:**
- PER z-score 분포 개선 시도. PER 분포가 극도로 우편향(max=13,170, 97.5%ile=340)
- PER=104가 z-score에서 "평균" 취급되는 문제 해결 목적
- → PER>80 하드필터가 더 직관적이고 효과적이라 폐기

**5-2. M 직교화 (G 잔차에서 M 계산):**
- G와 M이 소형 실적급등주에서 동시 극대화되는 문제 해결 시도
- M = M_original - slope × G 로 직교화
- 결과: slope=0.097 → 재표준화 후 G-M 상관이 거의 0. 직교화 불필요
- 근본 원인은 G-M 상관이 아니라 G와 M 각각의 극단값

**5-3. 균등 비중 (V25/Q25/G25/M25):**
- V10/Q25/G35/M30 → V25/Q25/G25/M25 시 브이엠 순위 확인
- v66 대비 큰 차이 없음. G 비중을 줄여도 G 자체가 극단값이면 여전히 지배적

**5-4. 12M-1M 모멘텀:**
- 6M → 12M-1M 변경 검토
- 2025년 5~6월부터 한국 장이 불장(~10개월) → 6M이 더 적절
- 12M-1M은 불장 초기에만 유리, 현재는 6M 유지가 맞음

### 핵심 발견: 소형 실적급등주 구조적 문제

**PER 분포의 비대칭:**
```
PER max = 13,170 → Winsorization 97.5%ile = 340
PER=104인 보성파워텍 → z-score에서 "평균" 취급
V_raw = -1.43 → 재표준화 후 V = -0.46 (패널티 거의 없음)
```

**G-M 이중 극대화 메커니즘:**
```
소형주 수주 1건 → 영업이익 48억→250억 (+420%)
→ G 팩터: 매출성장률 극단값 (+3.77~+4.72)
→ 주가 급등 → M 팩터: 6M 수익률 극단값 (+1.46~+2.50)
→ G(35%) + M(30%) = 65% 비중에서 둘 다 극대화
→ V(10%)의 PER 패널티(-0.46)로는 상쇄 불가
```

**실제 사례 (20260318 기준):**

| 종목 | 영업이익 | PER | V | Q | G | M | 합계 | 문제 |
|------|---------|-----|---|---|---|---|------|------|
| 보성파워텍 | 48→250억 | 104 | -0.06 | +1.67 | +4.03 | +1.61 | +2.30 | PER 104인데 V=-0.06 |
| 선익시스템 | 79→370억 | 37 | -0.46 | +1.25 | +4.72 | +1.46 | +2.35 | G=+4.72 극단 |
| 동아엘텍 | 선익시스템 자회사 | 16 | +0.57 | +1.01 | +3.77 | +0.42 | +1.31 | 모자회사 이중노출 |

**동아엘텍-선익시스템 관계:**
- 동아엘텍은 선익시스템의 모회사 (지분 보유)
- 두 종목이 동시에 상위권 = 실질적으로 같은 사업에 이중 노출
- 기존 상관관계 경고(corr > 0.7)로 감지되므로 별도 처리 불필요

### 전체 조합 테스트 결과 (v67 후보)

TTM/FY 매출성장률 + PER>80 필터 + ±2.5σ ceiling 동시 적용:

```
v67 후보 (20260318): scored 329개
순위  종목명              V      Q      G      M      합계
  1  브이엠            +0.03  +1.07  +2.50  +2.07   +1.77
  2  SK하이닉스         +0.01  +2.18  +2.50  +2.50   +2.17
  3  선익시스템          -0.46  +1.25  +2.50  +1.46   +1.58
  4  SK스퀘어          +0.34  +1.23  +2.50  +2.50   +1.97
  5  동아엘텍           +0.57  +1.01  +2.50  +0.42   +1.31
  6  삼성전자           +0.32  +1.12  +1.69  +2.50   +1.65
```

vs v66 원본:
```
v66 (20260318): scored 390개
  1  브이엠            +0.03  +1.07  +4.30  +2.07   +2.40
  2  SK하이닉스         +0.01  +2.18  +1.76  +3.06   +2.08
  3  SK스퀘어          +0.34  +1.23  +1.70  +3.06   +1.85
  4  선익시스템          -0.46  +1.25  +4.72  +1.46   +2.35  ← v66에서도 4위
  5  삼성전자           +0.32  +1.12  +1.50  +3.05   +1.75
```

### 폐기 이유

1. **TTM/FY 전환의 부작용이 이점보다 큼**: 선익시스템/동아엘텍 같은 소형 실적급등주가 구조적으로 상위 진입. PER>80 + ceiling으로 완화했지만 근본 해결 안 됨
2. **한 번에 너무 많이 바꿈**: 매출TTM/FY + 영업이익성장률 + PER필터 + ceiling + log(PER) + 직교화 등 6개 동시 변경 → 효과 분리 불가능. 유저 피드백: "한 번에 너무 많이 바꿨다"
3. **현행 v66 안정적**: Top 3(SK하이닉스/SK스퀘어/브이엠)이 24일 연속 유지. 연간 YoY가 소형주 극단값을 자연스럽게 억제
4. **개선 효과 불분명**: v67 후보 Top 5 중 3종목(브이엠/SK하이닉스/SK스퀘어)은 v66과 동일. 나머지 2개(선익시스템/동아엘텍)는 오히려 부정적

### 교훈 (향후 팩터 변경 시 참고)

1. **한 번에 1개만 변경하고 효과 측정** — 여러 변경 동시 적용 시 어떤 변경이 문제인지 알 수 없음
2. **영업이익성장률은 도입 금지** — 매출과 상관 높음 + Q 팩터와 이중 반영 + 기저NaN 33%
3. **PER 분포의 극단적 비대칭 인지** — PER max=13,170이면 PER=100도 z-score에서 "평균". Winsorization만으로 해결 불가
4. **G-M 이중 극대화는 구조적 한계** — G35+M30=65% 비중에서 소형 실적급등주 유리. V10%로는 상쇄 불가
5. **모멘텀 6M 유지** — 10개월 불장에서 6M이 적합. 12M-1M은 불장 초기용
6. **TTM/FY는 "더 최신"이지만 "더 노이지"** — 분기 실적 변동이 크면 연간이 안정적
7. **ceiling은 TTM/FY와 세트** — 현행 연간 YoY에서는 G 분포가 좁아서 불필요

---

## 핵심 변경사항 (v63 — 순위 기반 진입/퇴출 + Growth 부분결측 보정)

### 2026-03-18

**배경**: 시장 반등으로 MA120 통과 종목 급증(238→533), scored 유니버스 ~140→~300.
z-score 기반 절대점수(72/68)가 유니버스 크기에 민감하여 오작동.

**변경 1 — 순위 기반 진입/퇴출** (`ranking_manager.py`, `send_telegram_auto.py`):
- 진입: ✅ 검증 종목 중 가중순위 상위 5개 (ENTRY_RANK=5)
- 퇴출: 가중순위 15위 밖 (EXIT_RANK=15)
- 100점 환산은 표시용으로만 유지
- 순위는 유니버스 크기에 면역

**변경 2 — Growth 부분결측 0 처리** (`strategy_b_multifactor.py`):
- FwdPER 없는 종목: EPS개선도=NaN → 0(중립) 채움
- 단일 서브팩터로 G=4.0 받는 과대평가 방지

**변경 3 — exit_reason 순위 비교** (`ranking_manager.py`):
- 유니버스 크기 다른 날짜 간 z-score 직접 비교 폐지
- composite_rank 5위 이상 하락 시 "순위밀림"

**변경 4 — FnGuide/KRX 수정**:
- `refresh_fnguide_cache.py`: KRX 인증 추가, 병렬 4→2
- `krx_auth.py`: 중복 로그인 처리

**변경 5 — Top30→Top20 통일**:
- `get_stock_status()`, `compute_3day_intersection()` 기본값 20
- Watchlist 20종목 표시

**시행착오 (시도 후 롤백)**:
- percentile-z: 크기 정보 손실 (PER 200 패널티 소실)
- 시총 캡 200개: 중소형 우량주(브이엠) 탈락
- REF_MEAN/STD 보정: 기준값 staleness + 순위 못 고침
- MA120 3일 연속: 장기 랠리면 결국 같은 문제
- V15/G30 가중치: 순위 변동 거의 없음
- 극단 -1.5σ 탈락: 바이너리 클리프 (v44 철학 위배)
- FwdPER 미커버리지 -0.3: 정부 정책과 역행 + 근거 부족

---

## 핵심 변경사항 (v55 — 리스크 플래그 정리 + UI 문구 개선)

### 2026-03-10

**배경**: 가격 변동(급등/급락) 기반 리스크 플래그가 4겹 검증 통과 종목에 불필요. 미국 버전에도 가격 경고 없음.

**변경 1 — compute_risk_flags() 단순화** (`gemini_analysis.py`):
- Before: RSI≥80, 급락≤-5%, 급등≥8%, 거래량폭발 (4가지)
- After: RSI≥80만 유지 (+ tech_missing 데이터 누락 경고)
- 가격 변동은 시장 움직임이지 개별 리스크가 아님
- Gemini 프롬프트 위험 신호 설명도 RSI만 남김

**변경 2 — 데드코드 제거** (`send_telegram_auto.py`):
- `avg_market_chg`, `market_chg` 필드 제거 (시장 대비 초과수익률 계산용이었으나 불필요)

**변경 3 — Watchlist 하단 문구 개선** (`send_telegram_auto.py`):
- Before: "보유 종목이 상위 30 내라면 유지를 참고하세요" (부자연스러운 문장)
- After: "보유 종목이 상위 30 안에 있다면 유지해도 좋습니다"

**수정 파일**: `gemini_analysis.py`, `send_telegram_auto.py`

---

## 핵심 변경사항 (v54 — 모멘텀 6M 전환 + 이평선 이벤트)

### 2026-03-10

**배경**: 12M-1M 모멘텀이 현 시장 국면에서 구조적으로 오염됨.
1. 12M 룩백에 2025년 하반기 약세 구간 포함 → 정책 수혜 종목 신호 희석
2. 3/4 폭락으로 1M 수익률이 극단적 음수 → `12M-(-30%)` = crash 종목에 보너스 역설
3. 코스닥 정책 드라이브(2025.11~)가 정확히 6M 윈도우와 일치

**학술 근거**:
- Daniel & Moskowitz (2016): crash 후 4M Sharpe 0.83 vs 12M -1.57
- Goyal & Wahal (2015): 아시아에서 12M > 6M 증거 없음
- Mamais (2025): 위기 후 회복기에 짧은 윈도우 체계적 우위

**검증 과정**:
1. 블렌딩(12M-1M + 6M 평균) 테스트 → 턴오버 3배 증가 (2일 11개 vs 기존 4개). 두 신호 충돌이 원인
2. 6M 단독 테스트 → 턴오버 2배 (2일 8개). 블렌딩보다 안정적
3. Top 3(SK스퀘어, SK하이닉스, 브이엠) 두 방식 모두 동일 — 핵심 확신 종목 불변
4. 기존 유통/물류 내수주(신세계, BGF리테일, E1) 이탈, 반도체장비/바이오/로봇 진입 — 시장 테마와 일치

**변경**: `strategy_b_multifactor.py` `calculate_momentum()`
- Before: `(12M수익률 - 1M수익률) / 12M변동성` (LOOKBACK=252일)
- After: `6M수익률 / 6M변동성` (LOOKBACK=126일)
- 1M 차감 없음: 6M 윈도우의 1/6을 잘라내는 건 과도, MA120 필터가 하락추세 이미 제거
...
## 핵심 변경사항 (v53 — 이평선 이벤트 기반 전환)

### 2026-03-10

**배경**: 기존 이평선 경고는 매일 현재 상태(5일선↑/↓)를 표시하여, 폭락으로 MA가 왜곡된 경우 오해 유발 (예: -4.54% 급락인데 "5일선↑" 표시).

**변경**: 상태 표시 → 이벤트 기반. 이평선 돌파/이탈이 **발생한 날만** 표시.

| Before | After |
|--------|-------|
| `⚡ 코스닥: 5일선↑ 20일선↓` (매일) | `📉 코스닥 20일선 이탈` (이벤트 발생 시만) |
| `⚠️ 코스피: 5일선↓ 20일선↓` (매일) | (변화 없으면 미표시) |
| 아이콘: ⚡/⚠️/🚨 (하락 개수 기반) | 📈 돌파 / 📉 이탈 |

**로직**: 전일 종가 vs 전일 MA, 당일 종가 vs 당일 MA 비교 → 위→아래(이탈) 또는 아래→위(돌파) 전환 감지

**수정 파일**: `send_telegram_auto.py` — `_calc_market_warnings()` 함수
...
| # | 파일 | 내용 |
|---|------|------|
| 1 | `gemini_analysis.py` | `run_etf_matching()` 추가 — Pro 2-step (Step 1: 종목별 ETF 검색, Step 2: Greedy 최적 조합) |
| 2 | `send_telegram_auto.py` | 4번째 메시지 "📊 맞춤형 ETF" 추가 — Top 5 종목 기준 테마 ETF 2~3개 조합 |

#### ETF 매칭 구조
```
Step 1 (Pro + Google Search): Top 5 종목별 ETF 검색 + "복수 종목 포함 ETF" 크로스 레퍼런스
Step 2 (Pro, 검색 OFF): Greedy 알고리즘 — 커버리지 극대화, 중복 최소화
→ 2~3개 ETF 조합
```
...
## 핵심 변경사항 (v51.1 — 텔레그램 UI 개선, US v47.1 참고)

### 2026-03-08

**배경**: US 프로젝트 v47.1 전문가 패널 리뷰에서 3개 UI 개선 항목을 한국 프로젝트에 적용.

#### 변경 사항

| # | 개선 | 내용 |
|---|------|------|
| 1 | ~~Signal 시장 경고 배너~~ | 제거 — AI Risk에서만 표시 (Signal은 종목에 집중) |
| 2 | **이탈 사유 묶기** | Signal+Watchlist 이탈 알림에서 사유별 그룹 표시 (예: A·B(MA120↓) C(팩터↓)) |
| 3 | **Watchlist 면책 제거** | Signal에만 면책 유지, Watchlist는 1줄 안내로 교체 |
| 4 | **상관관계 문구** | "동일 섹터" → "주가 상관관계 높음" (실제 60일 상관관계 기반 표현) |
...
━━━━━━━━━━━━━━━
1. SK스퀘어(402340) · 금융
2. SK하이닉스(000660) · 전기전자
...
⚠️ SK스퀘어·SK하이닉스·삼성전자     ← 상관관계 경고
주가 상관관계 높음 — 이 중 1~2개 선택 권장

📋 선정 과정
...
📌 종목별 근거
...
📉 순위 이탈: A·B(MA120↓) C(팩터↓)   ← 사유별 묶기
```

---

## 핵심 변경사항 (v51 — Winsorized z-score + 과락 필터)

### 2026-03-08

**배경**: v50 Rank Percentile이 크기 정보를 소실하여 삼지전자(PER 5.2, 소형주)가 SK하이닉스(ROE 38.2%) 위에 랭크되는 문제. 또한 OCI(ROE -8.3%)가 Growth 극단값으로 Top 10 진입.

#### 정규화 진화 경로
z-score → MinMax[-1,1] (1위 독점) → Rank Percentile (크기 소실) → **Winsorized z-score + Renorm** (v51)

#### 변경 사항

| # | 개선 | 내용 |
|---|------|------|
| 1 | **Winsorized z-score** | 2.5%/97.5% 윈저화 후 z-score — 이상치 억제 + 크기 정보 보존 |
| 2 | **카테고리 재표준화** | 각 카테고리 std=1로 통일 — 서브팩터 수(V2/Q3/G2/M1) 차이로 인한 분산 불균형 해소 |
| 3 | **과락 필터** | 4개 카테고리 중 2개 이상 < -0.5σ → 제외 (OCI Q=-0.51, M=-0.87 → 2개 과락 제거) |
| 4 | **Growth 결측 페널티** | 둘 다 NaN → G=-0.5σ (Rank Percentile의 G=0.3에서 z-score 스케일로 전환) |
| 5 | **팩터 비중** | V25 + Q25 + G30 + M20 유지 (v48에서 확정) |

#### 과락 필터 설계 근거
- **-0.5σ (하위 ~30%)**: SK하이닉스(V=-1.24 1개만 과락)는 보존, OCI(Q=-0.51+M=-0.87 2개 과락)는 제거
- **2개 이상**: 단일 약점은 다른 팩터 강점으로 보완 가능, 복수 약점은 구조적 문제
- 20260306 기준: 248개→175개 (73개 과락 제외)

---

## 핵심 변경사항 (v48 — 팩터 비중 재조정 + UX 개선)

### 2026-03-06

**배경**: v47 테스트 중 모멘텀 팩터(25%)가 펀더멘털 0점 종목을 1위로 밀어올리는 문제 발견 (성호전자: V/Q/G 전부 마이너스, M=16.28로 1위). "6개월 급등 종목 사는 거면 아침에 급등종목 아무거나 사는 거랑 뭐가 다른가" — 모멘텀은 확인 신호여야지 주도 팩터가 되면 안 됨.

#### 변경 사항

| # | 개선 | 내용 |
|---|------|------|
| 1 | 사전필터 제거 | SKIP_PREFILTER=True, 전체 MA120 통과 종목 멀티팩터 채점 |
| 2 | 시총 커트라인 | MIN_MARKET_CAP 3000→1000억 (유니버스 939개) |
| 3 | FWD PER 미보유 | EPS개선도 NaN → 중앙값 대체 (neutral 처리) |
| 4 | 모멘텀 재설계 | (12M-1M)/Vol → 6M/Vol, 12M≤0 하드필터 제거 |
| 5 | Value 지표 축소 | PER+PBR+PCR+PSR+배당 → PER+PBR |
| 6 | z-score ±3σ 클리핑 | 전체 팩터 sub-factor에 적용 (극단값 지배 방지) |
| 7 | **팩터 비중 재조정** | V25+Q25+G25+M25 → **V30+Q25+G30+M15** (모멘텀 주도 종목 8/30→3/30) |
| 8 | 시장 동향 문단 분리 | Gemini 프롬프트에 2~3개 짧은 문단 지시 |
| 9 | 면책 문구 정리 | 항목별 분리, Signal/Watchlist 별도 범례 |
| 10 | 단기DC → 단기하락전환 | 약어 제거, 쉬운 한국어 |
| 11 | 선정과정 팩터 명시 | "멀티팩터 채점" → "가치·퀄리티·성장·모멘텀 채점" |

#### 설계 철학
- 하드 필터 최소화: MA120이 유일한 하드 필터, 나머지는 z-score 소프트 패널티
- **모멘텀은 확인 신호**: 15%로 펀더멘털 없이 모멘텀만으로 상위권 진입 불가. GARP에서 V+G가 핵심
- Value Trailing PER 유지: "지금 비싼가?" (Value) vs "이익이 개선되나?" (Growth) 역할 분리
- PCR/PSR/배당 제거: PCR은 Quality의 CFO와 중복, PSR은 PER과 상관 높음, 배당은 반성장 편향
- 상관관계 분산 선정 미도입: 스크리닝 도구이므로 비중/분산은 투자자 판단 (정보만 ℹ️ 표시)

---

## 핵심 변경사항 (v47 — 멀티팩터 시스템 개선, v48에 통합)

### 2026-03-06 (초기 버전, v48로 대체됨)

v47 변경사항은 위 v48 섹션에 통합. v47에서 V25+Q25+G25+M25 균등 가중이었으나, 모멘텀 주도 문제로 v48에서 V30+Q25+G30+M15로 조정.

---

## 핵심 변경사항 (v46 — 스크리닝 프레이밍 전환)

### 2026-03-03

**배경**: 텔레그램 메시지가 "추천" 프레이밍 사용 — "매수 후보 TOP 5 (각 20%)", "이탈 — 매도 검토", "매도를 검토하세요" 등. 시스템은 포트폴리오 비중을 계산한 적이 없으며, "각 20%"는 근거 없는 수치. 법적 리스크와 정보 정확성 양면에서 "스크리닝 결과" 프레이밍으로 전환.

#### 핵심 원칙
- **같은 데이터, 다른 프레이밍**: 멀티팩터 순위·AI 분석·시장 지표 등 모든 출력은 동일
- **행동 지시 제거**: "매수하세요", "매도를 검토하세요" → 정보만 제공
- **비중 미지정**: "각 20%" 제거 → "포트폴리오 비중은 투자자의 판단"

#### 메시지 프레이밍 변경

| 위치 | Before | After |
|------|--------|-------|
| Signal 인트로 | 유망 종목을 선별해 드려요 | 종합 점수 상위 종목입니다 |
| Signal 헤더 | 매수 후보 TOP 5 (각 20%) | **종합 점수 상위 5** |
| Signal 종목명 | SK하이닉스(000660) | **SK하이닉스(000660) · 전기전자** (섹터 추가) |
| 시장 정지 | 신규 매수 중단 | 🚫 시장 경고 — 스크리닝 일시 중단 |
| 빈 결과 | 무리한 진입보다 관망도 전략 | 3일 연속 상위권 유지 종목이 없습니다 |
| AI Risk 인트로 | 매수 후보의 위험 요소를 AI가 걸러냈어요 | 상위 종목의 리스크 요소를 AI가 분석했어요 |
| Watchlist 인트로 | 이 목록에 있으면 보유, 빠지면 매도 검토 | 상위 30종목과 순위 변동 현황입니다 |
| 이탈 헤더 | 이탈 — 매도 검토 / 매도를 검토하세요 | **📉 순위 이탈** (사유만 표시) |
| 콜드 스타트 | 매수 후보가 선정돼요 | 상위 종목이 표시됩니다 |
| 면책 | 참고용이며, 투자 판단은 본인 책임 | 멀티팩터 순위는 종목 선별 기준이며, 포트폴리오 비중은 투자자의 판단입니다 |

#### AI Risk 시장 지표 개선

**섹션 분리**: 시장 지수와 신용·변동성을 별도 섹션으로 분리
```
📊 시장 지수
🔴 코스피 6,244(-1.00%)
🟡 코스닥 1,193(+0.39%)
...
**약어 → 한국어** (credit_monitor.py `format_credit_compact()`):
| Before | After |
|--------|-------|
| HY 3.12% — 보통 | 회사채 금리차(미국) 3.12% — 주의 |
| 한국 BBB- 6.4%p | 회사채 금리차(한국) 6.4%p — 안정 |
| VIX 19.9↓ — 다소 높음 | 변동성지수(VIX) 19.9↓ — 주의 |

**색상·라벨 통일**: 🟢안정 / 🟡주의 / 🔴경고 (기존 "보통" + 🟡 모순 해소)

**액션 텍스트 제거**: AI Risk 메시지에서 행동 가이드 텍스트 미표시

#### Watchlist 섹터 분포

Top 30 종목의 섹터 분포를 1줄로 표시:
```
전기전자 6 | 기계 5 | 의약품 4 | 운수장비 3 | 기타 12
```

#### Top 5 팩터 기반 fallback 내러티브

**배경**: Gemini AI 실패 시 `_get_buy_rationale`이 PER 비교 기반 "실적 개선 (PER 37→6)" 같은 오해 소지 텍스트 생성.

**수정**: V/Q/G/M z-score 기반 팩터 강점 표시
```python
# z > 1.0 → "상위", z < -1.0 → "약세"
# 예: "밸류·퀄리티 상위 | 모멘텀 약세"
# 예: "멀티팩터 균형" (모든 팩터 z-score ±1.0 이내)
```

#### 섹터 대안 제거

반도체 집중 경고 + 대안 종목 섹션 전체 제거 (`SEMI_KEYWORDS`, `semi_tickers`, `_is_semi_related` 등). KRX 섹터 분류가 부정확하여 섹터 기반 경고가 오히려 혼란 유발.

#### TEST_MODE + --dates 복원

v45 롤백 시 손실된 TEST_MODE와 --dates 기능 재추가:
- `TEST_MODE=1` 환경변수 → 개인봇으로만 전송 (채널 보호)
- `--dates 20260227,20260226,20260225` → 테스트용 거래일 직접 지정
...
| 사유 | 조건 | 예시 |
|------|------|------|
| `120일선하락` | 현재가 < MA120 | 폭락장에서 이평선 이탈 |
| `거래부족` | 시총/거래대금 미달로 유니버스 탈락 | 소형주 전환 |
| `가치↓`/`모멘텀↓` 등 | 팩터 점수 하락 (T-0 vs T-1) | 실적 악화, 주가 급등 |
| `순위밀림` | 팩터 변화 미미, 다른 종목에 밀림 | 순위 경합 지역 |
...
## 핵심 변경사항 (v43.1 — 고객 친화 멘트 + 항상 TOP 5)

### 2026-02-26

**배경**: VIX 19.6 상황에서 Q3 멘트 "매수 축소, 급전환에 대비하세요"가 과격. 고객 입장에서 공포 유발.

#### 원칙
- **Q1/Q2**: 기회일 때 적극적 — 변경 없음
- **Q3**: 과격→신중 톤다운 — "급전환"/"매도 검토" 삭제
- **Q4**: 심각할 때 강하게 — 변경 없음
- **항상 TOP 5**: 시장 경고는 멘트로만 전달, 종목 수 축소(0/3) 안 함

#### final_action 메시지 전체 (US/KR 완전 동일)

| 구간 | VIX 안정 | VIX 높음 |
|------|----------|----------|
| Q1 회복기 | 적극 매수 구간이에요 (과거 연 +14.3%) | 변동성이 높지만, 분할 매수 유효 구간이에요 |
| Q2 성장기 | 정상 매수 구간이에요 (과거 연 +9.4%) | 매수 유지, 변동성에 유의하세요 |
| Q3 과열 초기(<60d) | 매수 유지, 시장 변화에 주의하세요 | 매수 유지하되, 신중하게 접근하세요 |
| Q3 과열 장기(≥60d) | 신규 매수는 신중하게, 보유 종목 점검하세요 | 신규 매수는 보수적으로 접근하세요 |
| Q4 침체 초기(≤20d) | 급매도 불필요, 관망하세요 | 매수 중단, 관망하세요 |
| Q4 침체 중기(20-60d) | 신규 매수 대기, 보유는 유지하세요 | 보유 비중 축소를 검토하세요 |
| Q4 침체 장기(>60d) | 회복 초입, 분할 매수를 검토하세요 | 바닥권 추정, 소액 분할 매수를 검토하세요 |

#### 코드 변경

| 파일 | 변경 |
|------|------|
| `credit_monitor.py` | Q3 4개 멘트 톤다운, Q2+VIX warn 통일, Q4 텍스트 US와 동일화, picks 전부 5 |
| `send_telegram_auto.py` | `market_max_picks = 5` 하드코딩 (시장 경고와 무관하게 항상 TOP 5) |
| US `daily_runner.py` | Q3 4개 멘트 톤다운, `portfolio_mode = 'normal'` 고정 (항상 TOP 5) |

#### 면책
- 모든 문장이 "~에요", "~하세요", "~검토하세요" — 정보 제공/제안형
- 기존 면책 문구 "참고용이며, 투자 판단은 본인 책임이에요"로 법적 커버 완료
...
**v42 가중순위 통일** — Top 30 판정·정렬·이탈을 모두 3일 가중순위(T-0×0.5 + T-1×0.3 + T-2×0.2) 기반으로 통일. 미국 프로젝트(eps-momentum-us)와 동일한 방식.

#### 가중순위 일관 적용
| Before | After |
|--------|-------|
| Top 30 판정: T-0 단일일 순위 | **3일 가중순위** 기준 |
| 이탈 판정: T-0 vs T-1 단일일 비교 | **가중순위 Top 30** vs T-1 비교 |
| Watchlist 정렬: T-0 단일일 rank | **가중순위** 기준 정렬 |
| 표시 순번: T-0 rank | **가중순위 위치** (일별 추이는 유지) |

#### 리스크 필터 정비
| Before | After |
|--------|-------|
| `vol_ratio: 1` 하드코딩 (죽은 코드) | 거래량 필터 **제거** (검증 종목의 거래량 폭발은 긍정 신호) |
| 기술지표 실패 시 RSI=50 (중립 통과) | **⚠️ 기술지표 확인 불가** 플래그 추가 |
| `compute_3day_intersection` 미사용 import | import **제거** |

#### 수정 파일
| 파일 | 변경 |
|------|------|
| `ranking_manager.py` | `get_stock_status()` 가중순위 기반 Top 30 + `get_daily_changes()` 가중 파이프라인 기반 이탈 |
...
|------|----------------|---------------|
| 이동평균 기간 | 60일 | **120일** |
| 버퍼 | 없음 (칼같은 이진 판단) | **없음** (MA120 이탈 시 즉시 제외) |
| 최소 데이터 | 60일 | **120일** |
| 함수명 | `apply_ma60_filter()` | **`apply_ma120_filter()`** |
| 조건 | `현재가 >= MA60` | **`현재가 >= MA120`** |

변경 이유:
1. **60일은 너무 짧음** — 2~3개월 단기 조정에도 걸려서 좋은 종목이 불필요하게 제외
2. **5% 버퍼 제거** — 120일선 이탈 시 칼같이 제외, 회복하면 자연 복귀
