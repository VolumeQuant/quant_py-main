...
[규칙]
- 종목별 2단계로 써:
  1단계 (필수): 왜 실적이 좋은지 / 어떤 사업이 성장 중인지 (핵심 비즈니스 드라이버). 이 단계는 반드시 써야 한다. 뉴스가 없어도 사업 매력은 생략 불가.
  2단계 (선택): {date_str} 전후 1~2주 이내 최신 뉴스가 검색되면 추가. 검색해도 최근 뉴스가 없으면 이 문장은 생략.
- ⛔ "검색되지 않습니다", "뉴스가 없습니다" 같은 부재 사실을 출력하지 마. 뉴스가 없으면 1단계만 쓰고 끝내.
