...
def flag_steps(tk):
    ...
    for rc in rcepts:
        ...
        avg4=op.iloc[-4:].mean()
        D=float(op.iloc[-1]/avg4) if avg4>0 else 1.0   # 최근분기/최근4Q평균 (붕괴=낮음)
        if ot<=0:
            steps.append((pd.Timestamp(rc).strftime('%Y%m%d'),0.0,0.0,D)); continue
        B=(nt-ct)/ast.iloc[-1]*100; C=op.iloc[-4:].max()/ot
        steps.append((pd.Timestamp(rc).strftime('%Y%m%d'),float(B),float(C),D))
    return steps
...
# ---- 표본검증 ----
print('\n=== 표본검증: B/C/D (PIT, 2026-06-05) — D=최근분기/최근4Q평균 ===')
for tk,nm in [('031330','SAMT(spike)'),('037460','삼지(spike)'),('171090','선익(collapse)'),('088130','동아(collapse)'),('080220','제주win'),('000660','SK win')]:
    bc=get_BC(tk,'20260605')
    print(f'  {nm}({tk}): {("B=%.1f C=%.2f D=%.2f"%bc) if bc else "데이터없음"}')
...
def build_crc(mode,bT,cT,drop=(),mult=0.7,dT=None):
    crc={}
    for d in ADATES:
        scored=[]
        for tk,sc,gs in RAW[d]:
            ...
            bc=get_BC(tk,d)
            spike = bc is not None and bc[0]>bT and bc[1]>cT          # v80.25: 최근분기 폭등
            collapse = (dT is not None) and bc is not None and bc[2]<dT  # 신규: 최근분기 붕괴
            flagged = spike or collapse
...
print('\n=== 최근분기붕괴(D) 페널티 스윕 — v80.25(B25/C0.7)에 D<dT 추가, G×0.24 ===')
print(f'{"config":<12}{"Cal":>7}{"CAGR":>7}{"WFmin":>7}{"2022":>7}{"2026":>7}{"turn":>6} | {"선익":>5}{"동아":>5}{"제주":>5}{"SAMT":>5}')
def line(tag,crc,c,cg,wf,y,tn):
    print(f'{tag:<12}{c:>7.3f}{cg:>6.1f}%{wf:>7.3f}{y.get("2022",0):>+6.1f}%{y.get("2026",0):>+6.1f}%{tn:>6} | {str(rk(crc,"171090")):>5}{str(rk(crc,"088130")):>5}{str(rk(crc,"080220")):>5}{str(rk(crc,"031330")):>5}',flush=True)
cb,cgb,mb,wfb,tnb,ehb,_=run(build_crc('none',999,999)); yb=yearly(ehb)
line('baseline',build_crc('none',999,999),cb,cgb,wfb,yb,tnb)
crc=build_crc('soft',25,0.7,mult=0.24); c0,cg0,m0,wf0,tn0,eh0,_=run(crc); y0=yearly(eh0)
line('v80.25만',crc,c0,cg0,wf0,y0,tn0)
for dT in [0.2,0.3,0.4,0.5]:
    crc=build_crc('soft',25,0.7,mult=0.24,dT=dT); c,cg,m,wf,tn,eh,se=run(crc); y=yearly(eh)
    line(f'+D<{dT}',crc,c,cg,wf,y,tn)
print('\n=== LOWO (−SK −제룡전기): D 페널티가 robust한가 ===')
for tag,dT in [('v80.25만',None),('+D<0.3',0.3),('+D<0.4',0.4)]:
    crc=build_crc('soft',25,0.7,mult=0.24,dT=dT,drop=('000660','033100')); c,cg,m,wf,tn,eh,se=run(crc); y=yearly(eh)
    print(f'  LOWO {tag:<10} Cal={c:.3f} CAGR={cg:.1f}% WFmin={wf:.3f} 2022={y.get("2022",0):+.1f}%',flush=True)
cD2,cgD2,_,wfD2,_,_,_=run(build_crc('none',999,999,drop=('000660','033100')))
print(f'  LOWO baseline  Cal={cD2:.3f} WFmin={wfD2:.3f}',flush=True)
print(f'\n총 {time.time()-t0:.0f}s')
