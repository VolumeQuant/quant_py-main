# -*- coding: utf-8 -*-
"""corpaction 효과 격리: production(state/) vs _sp0b(corp ON) vs _sp0b_nc(corp OFF). 같은 config V15Q0G55M30 12m E3X6S3.
corpaction OFF가 _sp0b를 production(3.83) 쪽으로 올리면 → corpaction이 gap 원인."""
...
arP, dP = load('state'); ar0, d0 = load('_sp0b'); arN, dN = load('_sp0b_nc')
...
print(f"  _sp0b_nc (corpaction OFF, 1.6): {cN:.3f}")
