...
for f in sorted(PEG.glob('ranking_*.json')):
    ...
    RAW[ds]=[(str(r['ticker']).zfill(6),(r.get('score',0.0) or 0.0),(r.get('overheat_pen',0.0) or 0.0),(r.get('growth_s',0.0) or 0.0)) for r in rows]
ADATES=sorted(RAW.keys())
univ=sorted({tk for d in RAW for tk,_,_,_ in RAW[d]})
...
def build_crc(mode,bT,cT,drop=(),mult=0.7,W_over=0.2):
    crc={}
    for d in ADATES:
        scored=[]
        for tk,base,pen,gs in RAW[d]:
            if tk in drop: continue
            sc=base+W_over*pen   # 과열캡 강도 W 적용 (pen<0=비쌈)
            bc=get_BC(tk,d)
...
# ③ 과열캡 강도 W 스윕 — 고PBR 모멘텀 펌프(제주080220/타이거219130) demote vs 수익. v80.25(B25/C0.7,G0.24) 유지.
print('\n=== 과열캡 W 스윕 (v80.25 유지) — 제주/타이거 고PBR펌프 잡고 수익 보존? ===')
print(f'{"W(과열캡)":<11}{"Cal":>7}{"CAGR":>7}{"WFmin":>7}{"2022":>7}{"2026":>7}{"turn":>6} | {"제주":>5}{"타이거":>6}{"SAMT":>5}')
def line(tag,crc,c,cg,wf,y,tn):
    print(f'{tag:<11}{c:>7.3f}{cg:>6.1f}%{wf:>7.3f}{y.get("2022",0):>+6.1f}%{y.get("2026",0):>+6.1f}%{tn:>6} | {str(rk(crc,"080220")):>5}{str(rk(crc,"219130")):>6}{str(rk(crc,"031330")):>5}',flush=True)
for Wv in [0.2,0.3,0.4,0.5,0.7]:
    crc=build_crc('soft',25,0.7,mult=0.24,W_over=Wv); c,cg,m,wf,tn,eh,se=run(crc); y=yearly(eh)
    line(f'W={Wv}',crc,c,cg,wf,y,tn)
print('\n=== LOWO (−SK −제룡): 과열캡 강화가 robust한가 (W0.2 vs W0.4) ===')
for Wv in [0.2,0.4]:
    crc=build_crc('soft',25,0.7,mult=0.24,W_over=Wv,drop=('000660','033100')); c,cg,m,wf,tn,eh,se=run(crc); y=yearly(eh)
    print(f'  LOWO W={Wv}  Cal={c:.3f} CAGR={cg:.1f}% WFmin={wf:.3f} 2022={y.get("2022",0):+.1f}%',flush=True)
print(f'\n총 {time.time()-t0:.0f}s')
