...
# ③ 가격 parabolic 가드용: 20일 수익률 (펌프=급등) — ADATES에 ffill 정렬
RET20=ohlcv.pct_change(20)
...
# ③ ret20 lookup (ADATES 문자열 키로 정렬)
RET20A=RET20.reindex(pd.to_datetime(ADATES),method='ffill'); RET20A.index=ADATES
def ret20(tk,d):
    if tk not in RET20A.columns: return None
    v=RET20A.at[d,tk]; return float(v) if pd.notna(v) else None
...
def build_crc(mode,bT,cT,drop=(),mult=0.7,para=None):
    crc={}
    for d in ADATES:
        scored=[]
        for tk,sc,gs in RAW[d]:
            if tk in drop: continue
            if para is not None:   # ③ parabolic 가드: 20일 +para%↑ 급등 종목 제외
                rr=ret20(tk,d)
                if rr is not None and rr>para: continue
            bc=get_BC(tk,d)
...
# ③ parabolic 가드 스윕 — 20일 +X%↑ 급등 제외. v80.25 유지. 제주080220/타이거219130 잡고 수익?
print('\n=== parabolic 가드 스윕 (20일 +X%↑ 제외, v80.25 유지) ===')
print(f'{"para":<12}{"Cal":>7}{"CAGR":>7}{"WFmin":>7}{"2022":>7}{"2026":>7}{"turn":>6} | {"제주":>5}{"타이거":>6}')
def line(tag,crc,c,cg,wf,y,tn):
    print(f'{tag:<12}{c:>7.3f}{cg:>6.1f}%{wf:>7.3f}{y.get("2022",0):>+6.1f}%{y.get("2026",0):>+6.1f}%{tn:>6} | {str(rk(crc,"080220")):>5}{str(rk(crc,"219130")):>6}',flush=True)
crc0=build_crc('soft',25,0.7,mult=0.24); c0,cg0,m0,wf0,tn0,eh0,_=run(crc0); y0=yearly(eh0)
line('가드없음',crc0,c0,cg0,wf0,y0,tn0)
for pa in [1.0,0.6,0.4,0.3]:
    crc=build_crc('soft',25,0.7,mult=0.24,para=pa); c,cg,m,wf,tn,eh,se=run(crc); y=yearly(eh)
    line(f'>{int(pa*100)}%제외',crc,c,cg,wf,y,tn)
print('\n=== LOWO (−SK −제룡): parabolic 가드 robust? (가드없음 vs >40% 제외) ===')
for tag,pa in [('가드없음',None),('>40%제외',0.4)]:
    crc=build_crc('soft',25,0.7,mult=0.24,para=pa,drop=('000660','033100')); c,cg,m,wf,tn,eh,se=run(crc); y=yearly(eh)
