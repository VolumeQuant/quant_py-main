...
print(f'방어 모드 G서브 서치: {len(defense_dates)}일', flush=True)

rk_mode = {d: rk[d] for d in defense_dates}
tsim = TurboSimulator(rk_mode, defense_dates, ohlcv, bench=bench)
...
for gs1, gs2 in g_sub_pairs:
    for gr in g_revs:
        for v, q, g, m in repr_weights:
            for mom in mom_types:
                r = tsim.run_fast(v/100, q/100, g/100, m/100, gr,
                                 entry_param=5, exit_param=8, max_slots=5,
        ...
        if done % 30 == 0:
            ...
            print(f'  [방어] {done}/{total} ({elapsed/60:.1f}분, 남은 ~{remain:.1f}분)', flush=True)

df = pd.DataFrame(results)
summary = df.groupby(['gs1', 'gs2', 'gr']).agg(
    avg_cal=('cal', 'mean'), max_cal=('cal', 'max'),
    avg_cagr=('cagr', 'mean'), count=('cal', 'count'),
...
df.to_csv(RESULT_DIR / 'v77_gsub_defense_all.csv', index=False)
summary.to_csv(RESULT_DIR / 'v77_gsub_defense_summary.csv', index=False)

print(f'\n방어 — 쌍별 Top 10:', flush=True)
for _, r in pair_best.head(10).iterrows():
...
try:
    ...
    top = pair_best.iloc[0]
    requests.post(f'https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage',
                  data={'chat_id': TELEGRAM_PRIVATE_ID,
                        'text': f'[v77 G서브 방어 완료]\n{top["gs1"]}+{top["gs2"]} cal={top["best_cal"]:.2f}\n소요: {(time.time()-t0)/60:.0f}분'},
                  timeout=10)
except Exception:
    pass
