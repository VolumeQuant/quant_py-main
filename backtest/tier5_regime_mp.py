"""Tier 5 — 국면 판단 격자 (MA period × confirm days)

Boost: v80.5 plateau (e2 sb5 SL-10 TS-8 gr0.5)
Defense: Tier 4 best (V35Q15G15M35 gr0.8 6m-1m sd4)
국면판단 변수:
  - MA period: 100, 150, 170, 200, 250
  - confirm days: 3, 5, 7, 8, 10, 15

총 5×6 = 30 조합. 단순 단일 워커로도 충분 (~50초).
"""
import sys, os, json, time
sys.stdout.reconfigure(encoding='utf-8')
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import pandas as pd, numpy as np
import requests
from pathlib import Path
from itertools import product
from turbo_simulator import TurboSimulator

PROJECT = Path(__file__).parent.parent

from config import TELEGRAM_BOT_TOKEN as BOT, TELEGRAM_PRIVATE_ID as PID
def send_tg(msg):
    if len(msg) > 4096: msg = msg[:4090] + '...'
    try:
        requests.post(f'https://api.telegram.org/bot{BOT}/sendMessage',
                      data={'chat_id': PID, 'text': msg, 'parse_mode': 'HTML'}, timeout=30)
    except: pass

print('=== Tier 5 — 국면 판단 격자 ===', flush=True)
t_start = time.time()

def load_rankings(dirs):
    data = {}
    for d in dirs:
        d = Path(d)
        if not d.exists(): continue
        for fp in sorted(d.glob('ranking_*.json')):
            k = fp.stem.replace('ranking_', '')
            if len(k) != 8 or not k.isdigit(): continue
            if k not in data:
                with open(fp, 'r', encoding='utf-8') as f:
                    data[k] = json.load(f)
    return data

boost_rd = load_rankings([PROJECT / 'state'])
defense_rd = load_rankings([PROJECT / 'state' / 'defense'])
all_dates = sorted(set(boost_rd) & set(defense_rd))
boost_rk = {d: boost_rd[d]['rankings'] for d in all_dates}
dates_74 = [d for d in all_dates if '20190102' <= d <= '20260512']

ohlcv = pd.read_parquet(PROJECT / 'data_cache' / 'all_ohlcv_20170601_20260512.parquet').replace(0, np.nan)
kdf = pd.read_parquet(PROJECT / 'data_cache' / 'kospi_yf.parquet')
kospi = kdf.iloc[:, 0].copy()
for c in kdf.columns[1:]:
    kospi = kospi.fillna(kdf[c])
kospi = kospi.dropna()

# MA 사전계산
MA_CACHE = {}
for period in [100, 150, 170, 200, 250]:
    MA_CACHE[period] = kospi.rolling(period).mean()

def calc_regime(target_dates, ma_period, confirm):
    ma = MA_CACHE[ma_period]
    reg = {}; md = False; stk = 0; ss = None
    for d in target_dates:
        ts = pd.Timestamp(d); kv = kospi.get(ts); mv = ma.get(ts)
        if kv is None or pd.isna(mv): reg[d] = md; continue
        s = kv > mv
        if s == ss: stk += 1
        else: stk = 1; ss = s
        if stk >= confirm and md != s: md = s
        reg[d] = md
    return reg

print(f'  데이터 로드 + MA 사전계산 ({time.time()-t_start:.1f}초)', flush=True)

# TSIM 1회 (모든 국면 격자 공유)
TSIM = TurboSimulator({d: boost_rk[d] for d in dates_74}, dates_74, ohlcv)
print(f'  TSIM 초기화 완료 ({time.time()-t_start:.1f}초)\n', flush=True)

# 고정 파라미터
BOOST = {'v':0.15,'q':0.0,'g':0.55,'m':0.30,'g_rev':0.5,
         'entry':2,'exit':6,'slots':5,'mom':'12m'}
DEFENSE = {'v':0.35,'q':0.15,'g':0.15,'m':0.35,'g_rev':0.8,
           'entry':3,'exit':6,'slots':4,'mom':'6m-1m'}
GS = ('rev_z', 'oca_z', None, None, None, None)

# 격자
MA_PERIODS = [100, 150, 170, 200, 250]
CONFIRMS = [3, 5, 7, 8, 10, 15]

results = []
total = len(MA_PERIODS) * len(CONFIRMS)
print(f'=== 격자 실행 ({total} 조합) ===', flush=True)
t0 = time.time()
for i, (ma_p, conf) in enumerate(product(MA_PERIODS, CONFIRMS), 1):
    reg = calc_regime(dates_74, ma_p, conf)
    boost_days = sum(1 for d in dates_74 if reg[d])
    try:
        r = TSIM.run_regime(defense_params=DEFENSE, offense_params=BOOST,
                            regime_dict=reg, trailing_stop=-0.08, stop_loss=-0.10,
                            g_sub1_o=GS[0],g_sub2_o=GS[1],g_sub3_o=GS[2],
                            g_w1_o=GS[3],g_w2_o=GS[4],g_w3_o=GS[5],
                            g_sub1_d=GS[0],g_sub2_d=GS[1],g_sub3_d=GS[2],
                            g_w1_d=GS[3],g_w2_d=GS[4],g_w3_d=GS[5])
        results.append({
            'ma': ma_p, 'confirm': conf, 'boost_days': boost_days, 'def_days': len(dates_74)-boost_days,
            'cal': r['calmar'], 'cagr': r['cagr'], 'mdd': r['mdd'],
            'sharpe': r['sharpe'], 'sortino': r['sortino']
        })
        print(f'  MA{ma_p} {conf}d: Cal={r["calmar"]:.2f} CAGR={r["cagr"]:.0f}% MDD={r["mdd"]:.0f}% (boost {boost_days}/{len(dates_74)})', flush=True)
    except Exception as e:
        print(f'  MA{ma_p} {conf}d: ERR {e}', flush=True)

wall = time.time() - t0
print(f'\n=== 완료: {wall/60:.1f}분 ===\n', flush=True)

df = pd.DataFrame(results)
df_sorted = df.sort_values('cal', ascending=False)

print('=' * 80)
print(f'{"순위":>3} {"MA":>4} {"conf":>5} {"boost":>6} {"def":>5} {"Cal":>6} {"CAGR":>6} {"MDD":>6} {"Sharpe":>7}')
print('-' * 80)
for i, (_, r) in enumerate(df_sorted.iterrows(), 1):
    print(f'{i:>3} {r.ma:>4.0f} {r.confirm:>5.0f} {r.boost_days:>6.0f} {r.def_days:>5.0f} '
          f'{r.cal:>6.2f} {r.cagr:>6.1f} {r.mdd:>6.1f} {r.sharpe:>7.2f}')

print(f'\n=== 변수별 평균 ===')
print(f'MA: {dict(df.groupby("ma").cal.mean().round(2))}')
print(f'confirm: {dict(df.groupby("confirm").cal.mean().round(2))}')

df.to_csv('C:/dev/_tier5_results_20260513.csv', index=False)
print(f'\n저장: C:/dev/_tier5_results_20260513.csv')

msg = f'<b>[Tier 5 — 국면 판단]</b>\n\n총 {total}조합\n\n<b>Top 5:</b>\n'
for i, (_, r) in enumerate(df_sorted.head(5).iterrows(), 1):
    msg += f'{i}. MA{int(r.ma)} {int(r.confirm)}d: Cal={r.cal:.2f} CAGR={r.cagr:.0f}% (boost {int(r.boost_days)}일)\n'
msg += f'\n현 baseline (MA170 8d): Cal {df[(df.ma==170)&(df.confirm==8)].cal.iloc[0]:.2f}'
send_tg(msg)
print('telegram sent')
