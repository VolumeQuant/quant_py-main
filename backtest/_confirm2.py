# -*- coding: utf-8 -*-
"""더블컨펌 임계 sweep — production rank<=Y ∩ sleeve top M, 분산되면서 엣지 유지되는 config 찾기."""
...
# 선행성장 랭크 사전계산 (state 유니버스 + 광범위)
fgrank={}
for d in rb:
    i=tdi[d];d1=tdays[min(i+250,len(tdays)-1)];fg=[]
    for t in cache:
    ...
    fg.sort(key=lambda z:-z[1]); fgrank[d]={t:r for r,(t,_) in enumerate(fg,1)}
def sim(PY,SM):
    picks={}
    for d in rb:
        prod=set(x['ticker'] for x in ar[d] if x.get('rank',99)<=PY)
        sl=set(t for t,r in fgrank[d].items() if r<=SM)
        picks[d]=list(prod&sl)
    ...
    for d in win:
        ...
        elif d in picks and picks[d]: held=picks[d]
        prev=d
    ...
    sh=a.mean()/(a.std() or 1)*np.sqrt(252);avgn=np.mean([len(picks[d]) for d in rb])
    return cagr,mdd,(cagr/abs(mdd) if mdd<0 else 0),sh,avgn
print("[더블컨펌 포트 sweep] production rank<=PY ∩ 선행성장 top SM(광범위)\n")
print(f"  {'config':16s}{'평균종목':>7s}{'CAGR':>7s}{'MDD':>8s}{'Calmar':>7s}{'Sharpe':>7s}")
for PY,SM in [(6,15),(10,30),(20,50),(20,100),(30,100),(30,150),(20,150)]:
    c,m,cal,sh,avgn=sim(PY,SM)
    print(f"  Y{PY} ∩ top{SM:<4}{avgn:>6.1f}{c:>6.0f}%{m:>7.1f}%{cal:>7.2f}{sh:>7.2f}")
print("\n  production 4.24/2.06 · 순수sleeve 1.4 · raw블렌드10% Sharpe2.15")
