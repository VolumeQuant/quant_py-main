...
LAST=ADATES[-1]
def srank(crc): return crc.get(LAST,{}).get('031330','권외')
print('\n=== 강도 정밀비교: baseline vs G×0.3 / 0.2 / 0.1 (B>25&C>0.7 일괄) ===')
cb,cgb,mb,wfb0,tnb,ehb,_=run(build_crc('none',999,999)); yb=yearly(ehb); wb=wf_blocks(ehb)
print(f'{"강도":<12}{"Cal":>7}{"CAGR":>7}{"WFmin":>7}{"2022":>7}{"2024":>7}{"2025":>7}{"2026":>7}{"SAMT(06-05)":>12}')
print(f'{"baseline":<12}{cb:>7.3f}{cgb:>6.1f}%{wfb0:>7.3f}{yb.get("2022",0):>+6.1f}%{yb.get("2024",0):>+6.1f}%{yb.get("2025",0):>+6.1f}%{yb.get("2026",0):>+6.1f}%{str(srank(build_crc("none",999,999))):>12}',flush=True)
for mult in [0.30,0.28,0.26,0.24,0.22,0.20]:
    crc=build_crc('soft',25,0.7,mult=mult)
    c,cg,m,wf,tn,eh,se=run(crc); y=yearly(eh)
    print(f'{"G×%.2f"%mult:<12}{c:>7.3f}{cg:>6.1f}%{wf:>7.3f}{y.get("2022",0):>+6.1f}%{y.get("2024",0):>+6.1f}%{y.get("2025",0):>+6.1f}%{y.get("2026",0):>+6.1f}%{str(srank(crc)):>12}',flush=True)
print(f'\n총 {time.time()-t0:.0f}s')
