...
def get_regime_params(mode):
    ...
    if mode == 'boost':
        return {
            ...
            # v80.24 (2026-06-08): EXIT_RANK 4→6 (과열캡 도입 후 재최적화)
            #   과열캡(v80.23)이 과열종목을 자동 강등 → 빡빡한 X4 매도가 중복(redundant).
            #   풀어서(X6) 과열 안 된 승자 더 오래 보유 → 수익↑ MDD↓ 회전↓.
            #   7.4년 BT(과열캡 baseline): Cal 3.84→4.43(+15%), MDD 27.1→25.9%, 회전 667→385(-42%).
            #   robust 그리드(LOO최악+IS+OOS) 1위, W×X 상호작용 W0.2/X6 정점, 인접 X5~8 plateau.
            #   LOO 최악절대값 X6 3.15 > X4 3.03 (과적합 아님). 진입3·슬롯3 불변.
