...
def get_regime_params(mode):
    ...
    if mode == 'boost':
        return {
            ...
            # v80.24 (2026-06-08): EXIT_RANK 4→6 (과열캡 도입 후 재최적화, 당시 브레드스 前)
            # v80.33 (2026-07-04): EXIT_RANK 6→5 (섹터 브레드스 방어 도입[6/20] 후 재최적화)
            #   ★계기: 사용자 "순위 가격민감도" → 실험 결과 가격민감도(당일 cr 이탈)는 무익/약세악화,
            #   진짜 레버는 '이탈 임계 엄격도'(가격무관). wr(3일평균) 유지, 임계만 6→5.
            #   ★규명: 브레드스가 약세 방어를 넘겨받아 이탈 최적점 6→5 이동. 브레드스 OFF면 X6 최적(=v80.24
            #   재현), ON이면 X5 최적. X5 우위는 브레드스 상호작용(OFF LOWO X5≈X6 ±0.05).
            #   faithful BT(브레드스 포함): Cal 4.55→4.85(+0.30), MDD −24.0 동일, 최근장 14.9→16.1,
            #   약세 0.84→0.81(거의 유지, X4는 0.69 붕괴라 기각). LOWO(ON) +0.33~0.59 robust, 인접CV 0.027.
            #   진입 E3·슬롯 S3 그리드 재확인 최적(불변). state 재생성 불필요(매매룰). research: backtest/_fastexit_faithful.py
            'ENTRY_RANK': 3, 'EXIT_RANK': 5, 'MAX_SLOTS': 3,
