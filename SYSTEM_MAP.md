...
| 항목 | v80 | **v80.6** |
|---|---|---|
| 시작점 BT | 2018-07-02 (7.8y) | **2019-01-02 (7.4y)** — 2018 H2 데이터 부족 제외 |
| 국면 | KP_MA170_8d | **KP_MA250_8d** |
| boost entry | 3 | **2** |
| boost slots | 3 | **5** |
| boost G_REV | 0.6 | **0.5** |
| defense V/Q/G/M | V30Q15G15M40 | **V35Q15G15M35** |
| defense G_REV | 0.7 | **0.8** |
| defense slots | 5 | **4** |
| 공통 TS | -15% | **-8%** |
| 공통 SL | -10% | -10% |
| 7.4y Cal | 2.38 | **3.99** (+68%) |
| CAGR | 103% | **143%** |
| MDD | 43.4% | **35.8%** |
| WF CV | 0.69 | **0.38** |

### 검증 (Tier 1~8, 5,000+ 조합)
- Tier 1: boost coarse 972 (entry/slots/SL/TS/gr)
- Tier 2: boost dense 1200 MP (Cal 3.50 plateau 발견)
- Tier 3: WF stability
- Tier 4: defense 512 MP
- Tier 5: regime 30 (MA250/8d Cal 3.99)
- Tier 6: 교차검증 75 MP (모든 regime에서 동일 최적 — 순서 의존성 없음)
- Tier 7+: MOM/VQGM/GSUB/ESX 40 (모두 baseline 최적 재확인)
- Tier 7B: 033100 의존도 48 MP
- Tier 8: 최종 WF + 인접 안정성 (CV 0.052 orig / 0.082 no033 PASS)

### 033100 의존
- v80.6: orig Cal 3.99 / no033 Cal 3.17 / delta 0.82 (21%)
- baseline (v80): orig 2.71 / no033 2.66 / delta 0.05 (2%)
- **v80.6 no033 환경에서도 baseline 능가** (3.17 > 2.71) — 진짜 알파, 운 아님
- v79 historical 9% 의존도 운용 가능 입증

### 데이터 정정 (2026-05-13)
- fs_dart 215종목 SG&A 매핑 정정 (5/12)
- 옵션F 폐기 (가짜 알파 제거)
- OHLCV 326종목 액면분할 미반영 → 옛 백업 복원 (수정주가)
- 204620 글로벌텍스프리 1종목 추가 보정
- KOSPI parquet 단일 'close' 컬럼 통합

### 백업 위치
- state_bak_v80_pre_v80_6_20260513/ (v80 직전 1929 file)

---

## v80.3 안전망: 이격도20 > 1.5 (2026-05-12)

**도입**: 매수 후보 진입 조건에 이격도20(20일 이평 대비) > 1.5 차단 추가.

**근거**:
- 옵션F + 섹터필터 제거로 KBI메탈(4연상한, +224%) 같은 종목 매수 후보 진입 가능
- 5/12 KBI메탈 -9.4% 사례로 추격 매수 위험 확인 (user 부모님 가입자 보호)
- 7.8년 BT (옵션F만): baseline Cal 3.937 → 안전망 Cal **4.117** (+0.18), MDD **-1.6%p**

**검증 (5/11 표본)**:
- KBI메탈 이격도20 = 2.26 → 차단 ✅
- 동아엘텍 (1.03), SK스퀘어 (1.47), SK하이닉스 (1.44) → 통과 (정상 모멘텀)

**대안 검토 (BT 결과 비교, 임계값 정렬)**:
| 안전망 | Cal | MDD | 평가 |
|---|---|---|---|
| baseline | 3.937 | 38.17% | 기준 |
| 이격도20 > 1.45 | 4.259 (peak) | 36.57% | 단독 peak — overfit 우려 |
| **이격도20 > 1.50** | **4.117** | **36.57%** | **채택 (robust plateau)** |
| 이격도20 > 1.60 | 3.979 | 38.17% | 보수, 알파 작음 |
| RSI > 85 | 3.846 | 37.16% | 알파 손실 |
| 이격도5 > 1.25 | 4.077 | 38.17% | 단기만, KBI 5/11 차단 안 됨 |

**구현** (`send_telegram_auto.py`):
- `get_stock_technical()`에 `disparity20` 추가
- `verified_picks` 루프에서 `disparity20 > 1.5` 종목 `🚫 이격도20 차단` 로그 + 제외
- Watchlist Top 20 표시는 그대로 (관찰)

## v80.2 → 롤백 (2026-05-12)

| 항목 | v80.2 (5/11) | rollback (5/12) |
|------|-----|-----|
| STOP_LOSS (양 모드) | -0.07 | **-0.10** (원복) |
| TRAILING_STOP (양 모드) | -0.10 | **-0.15** (원복) |

**롤백 사유**:
- 어제(5/11) v80.2 결정은 옛 데이터(옵션F+섹터필터) 기준 — Cal 4.149→4.261 (+0.11)
- 오늘(5/12) 섹터필터 제거 + state 7.8년 재생성 후 옵션F만 데이터로 BT 재실행
- 새 BT: baseline Cal **3.976** vs v80.2(SL-7/TS-10) Cal **3.646** = **-0.33** (방향 뒤집힘)
- 인접안정성 5x5도 baseline 우위 (plateau 3.42~4.05 vs 3.25~3.82)
- 원인: 산업지주사(SK스퀘어 등) universe 진입으로 변동성 큰 NAV 사이클 종목 늘어남 → tight stops가 잦은 손절로 알파 손실

**변경 파일**: regime_indicator.py, send_telegram_auto.py, CLAUDE.md

## v80 → v80.2 변경 요약 (2026-05-11, 5/12 롤백됨)

| 항목 | v80 | v80.2 |
|------|-----|-----|
| STOP_LOSS (양 모드) | -0.10 | -0.07 (rolled back) |
| TRAILING_STOP (양 모드) | -0.15 | -0.10 (rolled back) |

**근거**: 7.8년 BT regime-aware (Cal 4.149 → 4.261, MDD 35.73% → 34.89%) + 5x5 인접안정성 통과 (CV 0.020, gap +0.07).
**유의**: 옛 데이터(옵션F+섹터필터) 가정. 옵션F만 데이터에서는 무효화 → 5/12 롤백.
...
### BT 재검증 결과 (2018-07~2026-04 7.8y, 옵션F 단독)
| 지표 | 기존 | 옵션F만 |
|---|---|---|
| Calmar | 3.679 | **4.288** |
| CAGR | 142.1% | **156.1%** |
...
→ 옵션F만 단독 적용 시 BT 성능 전 지표 개선.

### BT 최종 결과 (옵션F + 섹터필터, 7.8y)
| 지표 | 기존 | 옵션F+섹터필터 (최종) |
|---|---|---|
| Calmar | 3.644 | **3.727** |
| CAGR | 140.8% | 135.7% |
...
→ 섹터필터 추가로 19종목(지주사) 제외 → Cal +0.083, MDD -2.2%p. **MDD 개선 + 중복 노출 방지 + 시스템 일관성** 우선. 옵션F만 vs 옵션F+섹터필터 차이 (Cal 4.288 → 3.727)는 지주사 미포함 트레이드오프.

### 옵션F가 드러낸 지주사 문제 + 섹터 필터 추가 (2026-05-12)

**문제**: 옵션F가 SK스퀘어(402340) 매출을 OFS=14,115 → CFS=104,556으로 정정 → ranking 5위 진입. 5/11 ranking에 산업지주사 19종목 진입 (SK스퀘어, LG, CJ, GS, HD현대, 에코프로, HD한국조선해양 등). SK하이닉스(자회사)와 동시 진입 → 같은 익스포저 중복.

**원인**: 옵션F 이전엔 OFS 오염으로 score 낮아 자동 탈락 → 시스템 결함 가려져 있었음. 옵션F 정정으로 결함 드러남.
- 기존 `EXCLUDE_KEYWORDS` (종목명 키워드 필터)는 "SK스퀘어"처럼 이름에 "지주/홀딩스" 없는 산업지주사 못 잡음
- KRX 분류상 산업지주사는 **"금융" 섹터** — 섹터 직접 필터 없음

**해결**: `fast_generate_rankings_v2.py` line 1604-1635에 KRX 섹터 "금융" 필터 추가
- 종목명 키워드 필터 직후 sector_map 빌드 + `sm_pre.get(t,'') == '금융'` 필터
- 5/11 표본: 325 → 306 (-19), 금융 섹터 0개. SK하이닉스/동아엘텍/선익시스템 등 본업 사업체 영향 없음
- 산업지주사 + 핀테크 + 투자조합 통합 제외

**State 재생성 (2026-05-12)**: 옵션F + 섹터필터 통합 적용으로 2018-07-02 ~ 2026-05-11 전체 재생성 (1929일)
- 이전 자의적 디자인(state 2021~, BT 2018-07~) 정정 → 시스템 일관성 위해 state도 7.8년 전체
- 사용자 정보: 2026-02 실가동, 그 전 state는 BT처럼 일괄 생성 (페이퍼 트레이딩)

**NAV 효과**: 지주사 디스카운트 해소 흐름은 시스템 모델 밖 — 별도 "지주사 워치리스트" 트랙 미래 디자인 가능
