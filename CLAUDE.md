...
# 🇰🇷 KR 전략 — quant_py-main (v79, 2026-04-15)

> 경로: `C:\dev\claude code\quant_py-main`
> 영구 지도: `C:\dev\SYSTEM_MAP.md` (전략 교체 시 맹점 제로 체크리스트)

## 국면전환 전략 (v79, 2026-04-15)

### v77→v78→v77→v77.1→v79 경위
...
- 2026-04-14 오후: 3-tier 시스템 (Crash Cash) 발견 → v77.1 (`f3ff8ddd0`)
- 2026-04-15 아침: 전수 이슈 조사 13개 (`fc29095d4`) + (d)+(d')+(e) 필터 도입 (`bac584104`)
- 2026-04-15 오전: PIT 수정 + BT 전체 재생성 (2018-07~2026-04)
- 2026-04-15 오후: **Phase 3~8 그리드 재탐색 → v79 확정** (c34a86390)

### v79 핵심 변경 (v77.1 대비)
- **국면: 5d → 7d** (Phase 5b 검증, 73% 7d 우세)
- **Crash Cash 제거** (v79 2f_rev_oca 방어가 이미 충분히 견고, trigger 0)
- **공격 재설계**: V5/G65 과도 → V15/G50 (값 반영 증가)
- **방어 재설계**: rev_accel+op_margin → **2f_rev+oca(0.7/0.3)** (v77 원래 단순 2팩터로 회귀)

### 2-tier 시스템 (v79: cash 제거)
1. **공격 (boost)** — KOSPI > MA200, **7일 확인** → Growth + Value
2. **방어 (defense)** — KOSPI < MA200, **7일 확인** → Momentum + Value + 2f(rev+oca)

### 국면 규칙 (KP_MA200_7d)
- **KOSPI > 200일 이동평균** = 공격, 미만 = 방어
- **7일 연속 확인** 후 boost↔defense 전환 (5d와 비교: 전환 횟수 동일, 타이밍 2일 지연으로 성과 향상)
- 모든 전환 시 기존 포트폴리오 **전량 청산**
- 전환 빈도: 7.8년 BT 21회, 5.25년 BT 14회 (5d와 동일 횟수)

### 공격 모드 (Boost) — KOSPI > MA200 (7일 확인)
- **V15 + Q5 + G50 + M30**
- G 내부: 3팩터 **rev_z 50% + oca_z 30% + gp_growth_z 20%** (v77.1과 동일)
- 모멘텀: **12m** (최근 1개월 포함, v77.1의 12m-1m → 12m)
- 진입: rank ≤ **3**, 퇴출: WR > **6**, 슬롯 **3**
- 손절: -10%, 트레일링: -15%

### 방어 모드 (Defense) — KOSPI < MA200 (7일 확인)
- **V30 + Q15 + G15 + M40**
- G 내부: **2팩터 rev_z 70% + oca_z 30%** (v77.1의 rev_accel+op_margin에서 회귀)
- 모멘텀: **6m-1m** (최근 1개월 skip, v77.1과 동일)
- 진입: rank ≤ 3, 퇴출: WR > 6, 슬롯 7
- 손절: -10%, 트레일링: -15%

### 성과 (v79 기준, BT 재측정)
**5.25년 BT (2021-01~2026-04)**: Cal=**3.18**, CAGR=105%, MDD=33%
**7.8년 BT (2018-07~2026-04)**: Cal=**3.20**, CAGR=121%, MDD=38%
- 7.8년 누적 수익: 40,808% (v77.1 재측정 3,794% 대비 10.8배)
...
### 표시 체계 (v77.1+, 2026-04-14) — 궤적(cr) + 점수(지수감쇠)
- **매매 로직**: 순위 기반 wr 유지 (BT Cal 4.45 유지, 변경 없음)
- **궤적 표시**: `r2→r1→r0위` 각 날짜의 **당일 cr(composite_rank)**
  - 예: 선익 4/14 "1→1→3위" = 그제 1등, 어제 1등, 오늘 당일 3등
  - 이전 "wr 정수순위" 방식 폐기 (당일 강도 변화 안 보였음)
- **점수 공식**: `score_100 = 100 × 0.9^(표시순번 - 1)` (Signal / Watchlist 공통)
  - 1위→100, 2위→90, 3위→81, 4위→73, 5위→66, 10위→39, 20위→14
  - 표시 순번(idx) 기반이라 **동점 발생 불가**
- 선정 이유:
  - 기존 `1위=100 / 꼴등=50` 강제 공식 폐기 → "선익 매일 100점" 현상 해결
  - wr 기반 공식에서 동점(4위/5위 같은 점수) 문제 → 순번 기반으로 해결
  - 궤적이 각 날짜 cr이라 **"오늘 약해졌다/강해졌다"** 체감 가능
  - 매 등수 10% 감소로 "1등 대비 몇 %?" 직관적
- 점수 기반 매매 BT: use_score_wr=True 시 Cal 악화 (7.8년 1.20→1.06, 5.25년 4.16→3.65) → 매매는 순위 기반 유지
