...
- **TTM PER/ROE 밸류 조사 → 기각, annual 유지 (2026-06-15)**: 사용자 "PER만 연간(pykrx)이라 불일치(PCR/PSR은 TTM)" 제기 → `USE_SELF_PER`(PER=시총/지배순이익TTM)+신규`USE_SELF_ROE`(ROE=DART TTM 우선)로 완전TTM 만들어 7.4년 BT(TurboSim 저장growth+overlay, **production 랭킹 100%일치 검증**, 풀그리드653 재최적+연도별WF+인접CV+LOWO). **annual 3.59 > 완전TTM best 3.01 (−16%), 연도별·인접CV(0.18)·LOWO(4종목 각 제외) 전부 annual 승**(단일종목 착시 아님). 이유: TTM PER은 최근급성장주를 싸게보여 **G/M과 중복**(이미 포착)→진짜승자 희석(특히 강세장); annual은 stale해서 G/M과 독립적·과열캡(v80.23)이 TTM변화 역할 = 역할분담. **두 플래그 default off 유지(재실험용, production 무변경).** 상세: `CHANGELOG.md` TTM 항목 + `backtest/_sp_final.py`·`_sp_lowo.py`
