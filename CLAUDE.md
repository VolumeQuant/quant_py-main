...
### 표시 체계 (v80, 2026-04-20) — 궤적(wr-rank) + 점수(wr 선형)
- **매매 로직**: 순위 기반 wr 유지 (BT: 순위 Cal=3.39 > 점수 Cal=2.62, v79 재검증)
- **궤적 표시**: `r2→r1→r0위` 각 날짜의 **당일 wr-rank** (= 그 날 wr 정렬 후 정수 순위)
  - T-0은 리스트 순번 = 궤적 순위 (wr 정렬과 일치)
  - 이전 표기(v79에 "당일 cr"로 기재된 건 오기. 실제는 항상 wr-rank)
- **동점 tie-breaker (v80 확정)**: wr이 같으면 **cr 작은 쪽(오늘 더 강한 종목) 우선**
  - 파일 생성/궤적 맵/Top 20 표시/진입 picks 전부 `(wr, cr)` 튜플 정렬로 통일
  - 2026-04-20 수정: 텔레그램 Top 20만 tie-breaker 어긋나 한화/코세스 순서가 파일과 반대로 표시되던 버그 발견 후 수정
