...
> ## ⛔ 검증 동결 — 2026-08-31까지 (2026-07-31 사용자 합의)
> 사용자: *"2월부터 7월말까지 맨날 알고보니 시스템에 오류가 있었다, 기준이 잘못 측정되었다,
> look ahead가 있었다. 계속 6개월동안 이 짓만 반복하네 지친다."*
>
> **하는 것**: 매일 신호 수신 / **실제로 안 돌아가는 버그만** 수정 / 8-14 반기실적 반영 확인 / 라이브 성과 누적
> **안 하는 것**: 재최적화 · 파라미터 재검증 · 새 필터 실험 · look-ahead 재감사
>
> **근거**: 6개월간 나온 "오류/look-ahead"는 대부분 **성적표 측정 문제이지 매매 신호 오류가 아니었다.**
> 핵심 엔진(국면 MA20/80/5일, V15Q0G55M30, 3슬롯, 진입 rank≤3)은 **불변**이고 바뀐 건 이탈순위(X4→X6→X5)와
> 보조 필터뿐. 계속 다시 들여다보니 매번 이전 측정의 흠이 나오고, 고치면 또 재측정해야 하는 무한 루프.
> 이미 6월에 "사전필터 추가 탐색 중단 권고"·"조기방어 전부 기각, baseline 최적" 결론이 나와 있었다.
> **시스템 문제가 아니라 운영 방식 문제.**
>
> **예외(즉시 처리)**: ①production이 실제로 깨진 버그 ②8/14 반기실적 파이프라인 이상.
> **보고 규칙**: 분석 결과는 **"매매 신호 영향 있음/없음"을 맨 앞에** 명시할 것. 영향 없으면 그게 결론이다.
> 메모리: `feedback_validation_freeze_2026_07_31.md`
...
> **★US 핸드오프 비판수용 — "revision > level" KR 미지지 (2026-06-26):** US 주장 = forward PER **레벨**은 사후/priced-in, 진짜 선행은 추정치 **리비전**(상향중인데 가격 미반영). **✅수용(방법론): 내 7.4년 레벨 BT는 look-ahead 오염**(forward EPS에 실제 미래실적 사용→레벨/리비전/priced-in 구분불가, 라이브 레벨은 과대평가됨). 신호real≠harvest·측정NOW 옳음. **⚠️반박(KR 데이터, US 본인규칙 "이식 가정 금지"): KR 15일 컨센 측정(n=2153) 레벨 IC +0.124 ≥ 리비전30 +0.098** = US와 반대(US는 좁은유니버스+상관0.83이라 레벨 priced-in, KR broad~1900은 레벨 아직 살아있음). 단 내 반박도 약함(15일·단일에피소드). **→ 둘 다 modest-real, 어느쪽도 미지배. 액션: 리비전을 `conviction_fusion_tracker`에 로깅 추가(비중 미반영, 60일+ 누적 후 레벨과 KR 시장서 직접 비교).** research: `backtest/_kr_revision_test.py`(IC측정).
> **★확신가중 CAP 5→3 + 제주 자작추정 폴백 (2026-06-26):** ①**CAP 5→3 재스윕**(선행PER<20 자격 기준, 구 cap5는 cross-sec 자격 스윕이라 stale): cap3 Calmar 4.92/MDD−26.1% > cap5 4.56/−28.8%, WF 전구간·LOWO 6종목 robust. 동일가중 대비 Calmar 3.92→4.92(+25%). 오늘 비중 SK60→47.7·제주 추가로 재배분. research `_conviction_prod_compare.py`·`_conviction_cap_validate.py`. ②**제주 자작추정**: 애널 컨센없는 held 종목에 자작 forward EPS 폴백(`self_forward_eps`=최근2분기 지배순이익 평균×4=보수적, lumpy 단일분기 과대반영 방지). 제주반도체 fwd_per 19.98(자작추정)으로 간신히 자격→기대성장+65%→비중 36.4%. ★held top3만 적용(잡주 무분별X), state/표시에 `self_est` 플래그. ⚠️razor's edge(19.98 vs 20)+자작이라 신뢰낮음. 제주는 어차피 production top3 매수확정, fusion은 비중제안만.
> **★별도 가치(gap) sleeve 기각 — US 핸드오프 검증 (2026-06-26):** US가 "KR은 gap↔모멘텀 직교(+0.015)라 별도 가치 sleeve가 분산 시너지 낼 것"이라 제안(US는 corr+0.83이라 시너지0). **KR도 기각.** 결합BT(look-ahead 상한): 가치sleeve 단독 Calmar **0.36~0.54·MDD−45%**(미래컨닝인데도 처참=기대성장 높은 종목만 동일가중=변동성 턴어라운드 범벅). 결합(70모멘텀/30가치)도 모멘텀 4.03→3.38로 **희석**(MDD만 −25.9→−23% 미세개선, Calmar↑&MDD↑ 진짜시너지 0회). **★직교만으론 부족 — sleeve 자체가 약함. gap은 종목선택(sleeve)이 아니라 비중조절(확신가중)로만 유효**(reweight 작동·selection 실패, 회사PC "38전38패"와 일관). research: `_value_sleeve_synergy.py`. US 핸드오프: `claude code/eps-momentum-us/research/KR_HANDOFF_value_sleeve_2026_06_26.md`.
> **★gap×fpe 자격 그리드 = US와 정반대 (2026-06-28):** US 그리드(공유): gap(=PER/fwdPER=기대성장) **2.5~3.5 컷 단독** 최고(+248), forward PER 컷 희석(+222). **KR 정반대**(`_sleeve_gap_fpe_grid.py`, look-ahead proxy): gap 자격컷 추가가 **해로움**(gap無 5.35→gap2.5 4.43→gap3.0 4.36), **gap無 + forward PER<20~25 자격이 최고(5.35)**. 이유: KR forward PER(밸류) IC 0.147>>기대성장 0.041 — **US=성장프리미엄(고성장 계속오름)→gap컷 유효 / KR=고성장주 이미 비싸·추격(고점)→gap컷=비싼거 사서 악화, 싼것(fwd PER) 우선**. **결론: KR 현행(fwd PER<20 자격 + 기대성장 비중[연속]) 정확, gap은 자격컷 금지. ★US 핸드오프 직수입 금지(시장구조 반대) — KR은 KR 데이터로 재검증.**
> **★유동성 필터(거래대금/거래량/회전율) 전부 기각 (2026-06-26, 사용자 "삼지·디바이스 작전 막자"):** ①**거래대금 바닥선 상향**: 매수진입 631건 거래대금<100억 코호트가 오히려 fwd20 +4.6%/큰손실3% > 두꺼움 +1.2%/9% = 얇은게 더 좋음. 2026 floor≥100억 시 수익 270→244%·MDD −24% 불변(낙폭은 얇은종목 탓 아님). 막으면 에스에이엠티+140%·2020디바이스+78%·브이엠(85억=얇음, 2026+28.4%) 같은 승자 학살. ②**회전율**: 패자 삼지4.08%·디바이스2.95% ≈ 승자 브이엠2.54%·에스에이엠티2.60%, 최고회전 제주10.75%(승자). 안 갈림. 유일 위험=회전율>15%(극단과열, n11, 0%승률) but 삼지/디바이스 정상범위라 미해당. **★삼지/디바이스 잡은 진짜장치=회계필터(accruals/lumpiness)지 유동성 아님.** research: `_liquidity_entry_test.py`·`_liquidity_floor_2026.py`·`_thin_winners.py`·`_turnover_test.py`. ③**EPS시스템 vs production 동일구간(16일)**: production E3X6S3 +35.6% ≫ EPS +3.0%(브이엠+79·SK+23 잡음). EPS 페이퍼 유지. ④★브이엠 V극단필터 강제이탈 관찰(+45%서 제외→+27%놓침)=고PBR모멘텀 휩쏘, 단 고모멘텀 V면제(E) 기각(위 -1.5σ 항목).
...
| 진입/퇴출/슬롯 | rank ≤ **3** / rank > **5** / **3슬롯** (E3**X5**S3, v80.33 — 브레드스 도입 후 X6→X5 재조정) |
...
- **메타 자산 배분 (사용자 영역, 2026-06-08 분기로 변경)**: 투자금의 **70% 시스템 + 30% 현금성**(MMF/RP/CMA/예수금). 리밸런싱 = **분기 첫 거래일(1·4·7·10월) 70:30 정기 + ±5%p 밴드**(분기 사이라도 시스템 비중 65~75% 이탈 시 즉시 리밸, 미세 드리프트 <2~3%p는 no-trade로 세금 절약). **매년→분기 변경 근거(BT)**: 70/30 블렌드(현금 연3%) 7.4년 시뮬에서 **매년 Calmar 3.56(주기 중 최악, MDD 23.4%) → 분기 4.14(MDD 19.2%) / 밴드만 4.15(MDD 18.4%)**. 밴드가 핵심엔진(필요시 정확 작동), 분기는 보조루틴(밴드 놓침 방지). 리밸 시 시스템 매도분 거래세 0.15% 드래그는 미미. 시스템 코드와 무관, 본인 메타 영역. US는 80:20(아직 매년, 미검증), KR이 더 보수적인 이유 = KR 약세장 깊이 + 트라우마 대응. 상세: `~/.claude/projects/C--dev/memory/project_cash_buffer_rule_kr.md`
  ...
  - **★★메타배분 확정 (2026-06-22, 사용자 결정 — 5년 50억 목표 공격운용)**: **현금 20% / 시스템 80%** 확정(70/30→80:20). 시스템 sleeve 내부 **KR 60 / US 40 시작 → US 라이브 검증되면(약세장 통과, ~1년+) 50:50** (US 현재 라이브 2026-02부터 N=1·2슬롯 집중이라 검증할인, KR은 7.4년·Calmar4.05·3슬롯이라 틸트). 총자산 = **48% KR + 32% US + 20% 현금**(→목표 40/40/20). **리밸=분기 첫거래일 + ±5%p밴드(시스템 75~85% 이탈시 즉시) + 2%p 무거래**. ★밴드가 자동 줍줍(시스템<75%→버퍼투입)/익절(>85%→버퍼환원), **재량 줍줍 금지**(밴드가 룰). **방어 2층**: ①시스템 자체 게이트(MA20/80)=동적 1차방어(약세 전환 자동현금화, 26년·6약세장 MDD-7~32p검증) ②현금20%=영구쿠션. **버퍼·KR/US비율은 regime 따라 안 옮김(고정)** — KR이 defense로 현금100%돼도 그 현금은 KR sleeve 대기(US로 이동 X, KR·US 위기 동반하락 corr 0.69~0.94이라 US도 약세).
    ...
    - **★섹터 쏠림 대응 + 확신가중 사이징 (2026-07-04)**: 매수권 top3가 전부 반도체(한국 시장구조)여도 **시스템 안에 억지 비반도체 끼우면 알파 깨짐**(순위 무시). 시스템은 상대강도 추종=반도체 강세 꺾이면 자동 로테이션. **쏠림 방어는 시스템 밖 3층**(3슬롯분산/브레드스50%/★메타 현금20·코어40·US). ★**"제일 좋은 종목(SK)에 몰빵" 금지** — ①반도체쏠림과 모순 ②3슬롯분산 알파 파괴(슬롯 40/40/20 약세 1.42→0.76 붕괴) ③CRWG 몰빵 패턴 재현. **비중=확신가중 그대로(SK48/제주36/티에스16), 감정 아닌 룰**. 브레드스 발동중이면 비중은 투자분(50%) 안에서 배분(SK48%=퀀트투자분의 48%지 총자산 아님). 최저비중 종목=시스템이 과열주에 덜 실은 것, 임의증액 금지. 상세: 메모리 `project_kr_semi_concentration_sizing_2026_07_04.md`.
  ...
  - **★★look-ahead 정정 — 위 "4.08→4.36"은 오염된 수치 (2026-07-31, 배포 `3601e376e`)**: US 레포 에이전트 지적 → KR 독립 재검증. **버그**: `calc_system_returns`가 **d-1→d0 수익의 노출 사이징을 d0 종가로 판정한 브레드스 상태로** 적용(미래 참조). BT 하니스 `_breadth_scale_bt.variant`도 `bdef[i]`를 `rets[i]`(=i-1→i 수익)에 적용해 **동일 오염 → 위 4.08→4.36 주장이 그 위에 서 있었음**. **수정**: 둘 다 d-1 기준(`_breadth_scale.get(d1)` / `[i-1]`). ★**매매 신호 불변, 표시 누적수익률만 정정(12566.1%→10854.6%, YTD 78.5%→65.0%, 당월 불변)**. **오염 크기**: X5 +0.53 / X6 +0.74(전환일 33/1860이라 제한적). 다른 PC 감사는 5%·내 측정 12~19% — **두 하니스 다 baseline 4.077 재현 실패**(state 재생성·쿨다운/SL 미반영)라 절대값은 어느 쪽도 권위 없음. 단 lag0>lag1>lag2 단조감소와 **브레드스 순이득 유지(X5 2.511→3.926)** 는 양쪽 일치 = **브레드스 도입 자체는 여전히 유효, 크기만 과대평가였음**. **★binary(전량현금) 전환 기각**: 오염 제거 후 전체는 binary 우위(4.179>3.926)라 "참여폭 뜨면 전량 현금화했어야" 보였으나 **배포 게이트 미통과 — WF 2019-21 강세 −0.515, LOWO −SK −0.202/−제주 −0.274**(전체 우위는 24-26 한 구간 +1.909이 만든 것). **현행 50% 스케일 유지**(인접 0.15~0.30이 4.37~4.39로 살짝 높으나 단일 하니스·plateau 내라 미채택). **국면(regime_by_date) 경로는 정상** — 포트폴리오는 d0 결정→d0→d1 반영, 전환청산도 equity 갱신 후 = 핸드오프의 "국면도 마찬가지"는 KR 미해당(★US 핸드오프 직수입 금지 재확인). research: `backtest/_breadth_lookahead_fix.py`(lag 0/1/2 × 50%/binary)·`_breadth_binary_validate.py`(WF+LOWO+인접).
  - **복귀 확인일 비대칭 재검증 = 현행 5일 유지 (2026-07-31, 사용자 "하루만에 벗어나는 게 낫지 않나")**: 국면 게이트 CONFIRM_DAYS를 exit(방어전환)×entry(공격복귀)로 분리해 재스윕(`backtest/_regime_recover_confirm.py`, 기존 `_regime_transition_search.py`가 entry 1일을 안 봤음). **복귀 1일이 최악권** — 전체 1.903 < 현행5일 1.956 < 3일 1.985, ★**약세 22-23에서 복귀1일 0.51 vs 3일 0.76 vs 5일 0.64**(하락장 가짜반등 휩쏘). MDD는 복귀일 무관 동일(−48.7%) = 복귀일은 손실크기 아닌 **수익만** 좌우. 3일이 미세 최적이나 +0.029는 노이즈(±0.10) 미만 → **변경 없음**. ⚠️ 이 하니스는 브레드스·쿨다운 미포함(절대 Calmar 1.9~2.3)이라 설정 간 상대비교 전용.
...
>
> **★lumpiness 추세면제(우상향 급성장 구제) 검증 = 기각 (2026-07-03, 사용자 "제주 매출 오락가락?" → 맹점 보완 시도):** 맹점 = lumpiness(min/max<0.25)가 min/max만 봐 우상향 급성장(제주 511→1110→918→1805, J커브)과 출렁(디바이스 53→262→86→392)을 못 구분. 보완안 = "최근분기=최대 AND 기울기>0이면 면제". ★상위20 표본(n=7) 우상향+12.7% vs 출렁−12.3%로 유망해 보였으나 **전체유니버스(n=3751)로 넓히니 우상향+5.4%≈출렁+6.4%(오히려 출렁↑), Δ−0.9%p = 면제 이득 0.** n=7은 소표본 착시(제주류 승자 우연). **면제 로직 기각, 현행 min/max<0.25 단순유지.** 제주는 개별 예외(0.28로 지금 통과, 0.25밑 되면 걸리나 룰로 구제 불가 = [[project_vm_valuefloor_hyoas_2026_06_26]] 브이엠 D모드 기각과 동일구조). ★교훈: n=7로 배포했으면 헛로직 추가, 넓은표본 검증이 막음(미검증 배포 금지). research: `backtest/_lump_uptrend_eda.py`.
>
...
**왜 3일가중이어야:** 당일 score로 하면 순위(3일 wr)와 기준 달라 어긋남(6위<7위 역전). 3일가중으로 맞춰야 ✅종목 완전 정합. + footer 줄간격 compact(매수/매도/시스템 빈 줄 제거). 사용자 "점수는 솔직하게"(앵커 1.9 유지, 강해지면 80점대).
**★결측일 처리 수정 (2026-07-06, 사용자 "케이씨텍 0.0점/삼지 8점 그러면 안되지"):** 구 룰(빠진 날=그날 50위 score 폴백)의 두 결함 수정 — ①필터 제외일 sentinel(score -999)이 가중평균에 섞여 0.0점 붕괴(케이씨텍 7/1 밸류바닥 제외 → -999×0.25) ②50위 score가 음수(-0.37)·날마다 변동이라 신규주 과다 감점(삼지 8.0점). → **-999는 결측 취급 + 결측일은 있는 날 가중치로 재정규화**(0.4/0.35/0.25 비율 유지). 케이씨텍 0.0→34.1, 삼지 8.0→22.8, ✅종목(3일 전부 존재)은 소수점까지 불변=회귀 없음. 표시 전용·매매 무관(`ranking_manager.weighted_score_100`).
...
## v80.35 SL −15% 재도입 (테일 보험) — 2026-07-08 (배포됨, 사용자 승인)

> 배포: `regime_indicator.py` boost/defense `STOP_LOSS: None → -0.15` + `send_telegram_auto.py` SL 이탈도 재진입 쿨다운 기록(★필수 — 안 하면 순위 높은 채 SL 맞은 종목이 다음날 재매수 루프, 제주 유형) + 매도룰 문구 2곳. 스모크: 보유 1종목(SK)·제주 SL 청산·쿨다운 10일 등록 확인. 롤백: `STOP_LOSS=None` + git revert.
> **계기**: 제주 보유분 −24.9%(순위 2위 유지한 채 폭락 = 순위이탈이 못 잡는 X5 체제의 유일한 구멍) → 사용자 "−30% 트리거 기다리면 늦다, 미리 재검증해라". v80.22의 "단일거래 −30% 시 SL 재검토" 트리거를 이 룰이 대체.
> **재검증(`_sl_revalidate_20260708.py`, X5+쿨다운10+브레드스 하니스)**: ①타이트 SL(−8/−10%)은 여전히 손해(4.60/4.85<5.15) = v80.22 기각 유효 ②**−15% = 비용 0 테일 보험**: 전체 Calmar 5.15 동일, 최악 단일거래 **−34.9%→−19.9%** 절단, 브레드스 무시 기준 전체 +0.14·2026 +1.9. 약세22-23 −0.11 소폭 비용. LOWO 6종목 전 셀 동등~우위(−제룡 +0.23), 인접 −12/−15/−20 plateau(−12는 LOWO −SK −0.29 취약 → −15가 어깨). ③채택 근거 = 수익 아니라 보험(헤드라인 개선 0 = 노이즈 미달임을 명시 라벨링). ⚠️종가 기준 측정 = 실전 갭 체결은 −17~20% 가능. ⚠️v80.22와 다른 결론인 이유 = X6→X5 환경변화(타이트 SL 중복→−15%는 순수 테일컷). 표시 수익률 리플레이 변동(YTD 214.4→215.7)은 정상.
...
> **나머지 8월 시즌 방어 코드 확인 완료**: 폭주=list API 3일창+타임아웃 5h, H1 타겟매핑(FILING_SEASON[8]=06-30), 정상경로 반기 단일값 정확(실측), 페널티 3종 전부 PIT(rcept_dt≤base_ts), 누락감지 개인봇 알림(v80.8), FnGuide 컨센 붕괴는 6/30 WISEfn 전환으로 기복구. research: `backtest/_h1_doc_fallback_probe.py`·`_h1_doc_ctx_dump.py`.
> **+SPOF P1 수정 (2026-07-07 동일 배치)**: `run_daily._is_trading_day` — (구) pykrx **빈 응답**(IP 차단 전형)을 '휴장일' 오판 → 신호 무소음 결번. 수정: 빈 응답+평일이면 **yfinance KOSPI(^KS11) 교차확인**(KRX 독립 소스) + 전 경로 개인봇 알림(무소음 제거), 실패방향=**진행 우선**(휴장일 오진행=중복메시지 무해 < 거래일 결번=신호 유실). 4경로(정상/차단의심/휴장일/판단불가) 모킹 테스트 PASS.
> **+SPOF P2 2건 수정 (2026-07-07 동일 배치, SPOF 감사 전 항목 🟢 완료)**: ①**kospi 캐시 staleness 경보** — `run_daily._check_kospi_staleness`(data_refresher 직후): 인덱스 캐시가 3영업일+ 뒤처지거나 부재면 개인봇 경보(국면 MA 동결 감지, 비차단. 수일은 5일 확인 룰이 완충—장기화가 위험). ②**브레드스 실패 명시화** — `breadth_diagnostic.build_breadth_advisory` 실패 시 빈문자열(조용한 소멸) 대신 "⚠️ 시장 참여폭 계산 실패 — 어제 상태 기준으로 판단하세요" 라인(X5가 브레드스 규율 전제라 발동 중 소멸이 위험, 킬스위치 DISABLE만 빈문자열 유지). 모킹 테스트 5/5 PASS(스테일/부재/브레드스 정상·실패).
...
**계기:** 사용자 "순위가 가격에 더 민감하게" → 가격민감도 실험(당일 cr 이탈) 결과 **가격민감도 자체는 무익/약세악화**(2x2 분해: 속도축 wr→cr은 임계6서 +0.17·임계5서 −0.11 부호역전=불안정). 사용자가 "이탈 빡빡하게 = 가격민감도 아니라 순위민감도"라 정확 지적 → **진짜 레버는 '이탈 임계 엄격도'(가격무관)**로 판명.

**★핵심 규명 — 브레드스가 최적점 6→5 이동시킴:**
| 이탈임계 | 브레드스 ON(현재) | 브레드스 OFF(v80.24 당시) |
|---|---|---|
| wr>4 | 4.74 | 4.20 |
| **wr>5** | **4.85** ← 현재 최적 | 4.19 |
| **wr>6** | 4.55 | **4.29** ← v80.24 재현 |
| wr>7 | 4.05 | 3.76 |

- 브레드스 **OFF**면 X6 최적(4.29)=**v80.24 결론 정확 재현**(당시 브레드스 前이라 X6 맞았음). **ON**이면 X5 최적(4.85). 브레드스(6/20 도입)가 약세 방어를 넘겨받아 이탈을 다시 빡빡(5)하게 해도 안전해짐. X5 우위(+0.30)는 순전히 **브레드스 상호작용**(OFF LOWO X5≈X6 ±0.05).
- **★★브레드스 규율 의존 (사용자 지적, 배포 전제):** X5는 브레드스 50% 축소를 **실제 지킬 때만** 유효. 지킴 X5 4.85>X6 4.55(+0.30) / **안지킴 X5 4.19<X6 4.29(−0.10, 특히 강세장 −0.26)**. 브레드스는 advisory(수동)라 발동(전체 35%)시 50% 현금화 규율이 X5 배포의 필수 전제. 안 지킬 거면 X6 유지가 나음.

**검증(브레드스 포함 faithful, E×X×S 그리드):** 진입 E3 지배(E2/E4 열위)·슬롯 S3 최적(S4/S5 전체↓)·이탈 X5 스위트스팟(X4는 약세 0.84→0.69 붕괴 기각). E3X5S3 전체 4.55→4.85(+0.30), 최근 14.9→16.1, MDD −24.0 동일, **약세 0.84→0.81 거의유지**(X4와 달리 방어). LOWO(ON) +0.33~0.59 robust, 인접 CV 0.027. 오늘(7/3) 표본: wr 5~6 사이 종목 없어 당장 매매변화 0(안전배포). research: `backtest/_fastexit_faithful.py`·`_wr_smoothing.py`·`_price_sensitivity_grid.py`(가격민감도 기각 경위).
...
- **-1.5σ 단일팩터 바닥 필터**: V/Q/G/M 4팩터 중 하나라도 -1.5σ 미만이면 제외. baseline(유지)이 BT 최선 (완화/제거 모두 성과 하락). **부작용**: 주가 급등으로 PER/PBR 비싸진 대형 전력주(HD현대일렉트릭, 제룡전기 등)가 V -1.5 미만으로 탈락. `EXTREME_MODE` env로 A/B/C/D 실험 가능 (미설정=baseline)
  - **★고모멘텀 V면제(EXTREME_MODE=E) 기각 (2026-06-26)**: 사용자가 브이엠(고PBR 모멘텀) 급등 중 V<-1.5로 강제이탈(06-10 +45%서 제외→이후 +27% 놓침) 관찰 → "V<-1.5라도 고모멘텀(M≥1.0)이면 면제하자" 제안. **샘플(브이엠)은 rank1로 부활하나, 7.4년 표본검증서 기각.** ★검증법: `EXTREME_MODE=C`(필터OFF 전종목) 전체범위 1회 재생성 → -1.5σ/E 재필터링(재생성 비결정성 통제, 샘플 4일 top5 5/5 production 정확재현). **①E 코호트(V<-1.5&M≥1.0&Q/G≥-1.5) forward60: 평균+2.5%/승률42%/중앙 -4.8% < 정상통과 +4.2%/45%/-2.7%** = 살리는 종목이 코호트 전체로 부진(많이 올라 비싼 종목은 반락多, 브이엠은 운좋은 예외). **②3슬롯 BT: baseline Calmar 4.10→E 3.40, MDD -26.4→-30.0%, 전 구간(약세 0.91→0.78 포함)·LOWO(-브이엠/-제룡) 전부 E 열위.** = -1.5σ 완화는 어떤 형태든(A/C/D/E) 성과 하락 재확인. 브이엠 휩쏘는 net이득 필터의 비용(가끔 승자 토함)이지 버그 아님. E코드는 실험훅으로만 잔존(기본off). research: `backtest/_Eanalysis.py`·`_regen_modeC_full.py`(mode C 재필터링 BT 기법), `_Ecohort_validate.py`.
