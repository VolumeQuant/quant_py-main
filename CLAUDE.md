...
> **★강도 차등 (2026-06-25, 사용자 "강할수록 더 사기"):** binary(확인=일률×3) → **grow 비례** `w=min(1+CONV_K×(기대성장−1), CONV_CAP)` (기본 K2/CAP5). BT(look-ahead 상한): binary×3 Cal 4.35 → grow비례 k2cap5 **4.57(+0.22)**, k1cap3 4.39, 순위비례·계단식은 binary 미달(grow값 비례가 최적). ⚠️이득은 집중(cap↑)에서 — SK하이닉스 42.9→60%로 한 종목 쏠림(3슬롯 분산 약화), MDD 동일. 표시제안만(사이징 본인판단). env `FUSION_CONV_K`/`FUSION_CONV_CAP`. research: `backtest/_conviction_strength.py`.
