...
### 표시 점수 공식 (v77.1+, 2026-04-14) — 지수 감쇠
- **매매 로직은 순위 기반 wr 유지** (BT Cal 4.45 유지)
- **표시 점수는 wr 기반 지수 감쇠** (Signal / Watchlist 동일 공식):
  - `score_100 = 100 × 0.9^(wr - 1)` (cap 0~100)
  - wr=1→100점, wr=2→90, wr=3→81, wr=5→66, wr=10→39, wr=20→14
- 선정 이유:
  - 기존 상대값 공식(`1위=100 / 꼴등=50` 강제) 폐기
  - 1위 점수가 wr 값에 따라 매일 변동 ("선익 매일 100점" 현상 해결)
  - 순위 ↔ 점수 항상 일치 (wr 낮을수록 점수 높음, 역전 없음)
  - 매 등수 10% 감소로 직관적 ("1등 대비 몇 %?")
- 점수 기반 BT 검증 결과 (참고): use_score_wr=True 시 7.8년 Cal 1.20→1.06, 5.25년 4.16→3.65 (열등). 매매는 순위 기반 유지.
