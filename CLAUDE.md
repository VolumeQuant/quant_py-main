...
> **★강도 차등 (2026-06-25, 사용자 "강할수록 더 사기"):** binary(확인=일률×3) → **grow 비례** `w=min(1+CONV_K×(기대성장−1), CONV_CAP)` (기본 K2/CAP5). **K×CAP 스위트스팟 스윕(`_conviction_capsweep.py`): CAP=5가 정점(plateau 4~6, Cal 4.50~4.58), CAP=2/3는 명백 열위(4.23/4.40), CAP=8은 과집중 하락(4.43). K는 1.5~3 무차(plateau)라 K2 유지.** ⚠️사용자 "3배"는 구 binary 스위트스팟(×3) 발언이었고 강도차등엔 부적용 — 차등은 cap5가 스위트스팟. ⚠️이득은 집중(cap↑)에서 — SK하이닉스 60%로 한 종목 쏠림(3슬롯 분산 약화), MDD 동일. 표시제안만(사이징 본인판단). env `FUSION_CONV_K`/`FUSION_CONV_CAP`. research: `backtest/_conviction_strength.py`·`_conviction_capsweep.py`.
