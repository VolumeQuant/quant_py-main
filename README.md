...
삭제 (v76~). 백테스트에서 성과 하락 확인.

### 순위 체계 (v77)

- `composite_rank`(cr): 당일 단독 멀티팩터 순위. **판단 기준으로 안 씀.** wr 계산 입력값 + 궤적 표시(r2→r1→r0)용.
- `weighted_rank`(wr): `cr_t0×0.5 + cr_t1×0.3 + cr_t2×0.2`. **모든 판단의 유일한 기준.**
- postprocessing 후 `rank` = wr 기준 정렬 순위.

### 진입/퇴출 (v77, 국면전환)

| 구분 | 기준 | 공격 모드 | 방어 모드 |
|------|------|-----------|-----------|
| **진입** | ✅ 종목 중 wr 상위 | entry_rank 7 | entry_rank 3 |
| **퇴출** | wr 값 > exit_rank | 8 | 6 |
| **손절** | 진입가 대비 | -10% | -10% |
| **트레일링** | 고점 대비 | -15% | -15% |
| **슬롯** | 동시 보유 상한 | 3 | 7 |
...
2. weighted_rank 기준 Top 20 모니터링
3. 3거래일(T-0, T-1, T-2) 연속 Top 20(rank ≤ 20, wr 기준)에 포함된 종목만 후보 자격 부여
4. 후보 중 wr 상위 entry_rank개가 매수 후보
5. 상태 표시: ✅ 3일 검증 / ⏳ 2일 관찰 / 🆕 신규 진입
...
- 상위 20종목 점수 정렬 + 순위 궤적
- 섹터 분포 1줄 요약
- `── 매도 검토선 ──` 구분선 (wr > exit_rank 기준)
- 매매 조건 표시 (매수: ✅ 상위 entry_rank / 매도: wr>exit_rank or 손절)
- v77: 공격 E7X8S3, 방어 E3X6S7
- 순위 이탈 (사유별 묶기)
