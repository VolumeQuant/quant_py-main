...
> 채널: [@kr_dailyquant](https://t.me/kr_dailyquant) | 전략 버전: v69 | 최종 업데이트: 2026-03-22
...
| 팩터 | 비중 | 서브팩터 | 정규화 |
|------|------|----------|--------|
| **Value** | 25% | PER + PBR + PCR + PSR | 전체 유니버스 rank z-score (낮을수록 좋음) |
| **Quality** | 25% | ROE + GPA + CFO | 전체 유니버스 rank z-score |
| **Growth** | 25% | 매출성장률 YoY | 전체 유니버스 rank z-score (커버리지 96%) |
| **Momentum** | 25% | 6M수익률/변동성 + K_ratio | **섹터 내** rank z-score (추세 제거) |
...
| 구분 | 기준 | 설명 |
|------|------|------|
| **진입** | score_100 >= 72 | 3일 검증 종목 중 72점 이상 전종목 (가변 수) |
| **퇴출** | score_100 < 68 | Watchlist 매도 검토선 (4pt 버퍼) |

- `score_100 = (weighted_score + 0.7) / 2.4 x 100`
- `weighted_score = T0 x 0.5 + T1 x 0.3 + T2 x 0.2` (3일 가중)
- 비중은 미지정 (투자자 판단)
...
3. 후보 중 score_100 >= 72인 종목이 매수 후보
4. 상태 표시: **V** 3일 검증 / **?** 2일 관찰 / **NEW** 신규 진입
...
- 매수 후보 종목 (score_100 >= 72, 가변 수)
- 상관관계 경고 (corr > 0.7)
- 선정 과정 퍼널 (유니버스 -> 채점 -> 검증 -> 기준점수)
