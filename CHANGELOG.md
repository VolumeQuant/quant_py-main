<!--
이 파일은 CLAUDE.md 슬림화(2026-05-28) 이전의 전체 아카이브입니다.
버전 변경 이력(v75~v80.20), 거부된 옵션, 과거 사고 상세, 검증 단계 기록을 보존합니다.
현재 운영 기준(현재 production 파라미터 + 운영 규칙)은 CLAUDE.md를 참조하세요.
이 파일은 자동 로드되지 않으므로, 과거 결정의 근거/맥락이 필요할 때만 검색해서 읽으면 됩니다.
-->

# [아카이브] 전체 변경 이력 (CLAUDE.md 슬림화 이전 스냅샷, 2026-05-28)
...
### 5/27 후속 검증 — 4건 거부 (변경 없음)

**1. 거래대금 임계 강화 (공매도 표적 회피)** — 거부
- 20일 평균 ≥ 20억 → 30/50/100/200/300억 grid
- 결과: 임계 ↑ = Cal 명확 감소 (30억 -16% / 50억 -17% / 100억 -42% / 300억 -73%)
- MDD 동일 또는 악화 (100억+ 임계서 MDD ↑)
- 시스템 알파의 본질 = 거래대금 작은 중소형 폭등 종목 잡기 — 임계 강화 = 알파 핵심 제거
- 사용자 우려 (타이거일렉 28억 등) 정당하지만 시스템 룰 변경은 큰 손해

**2. ETF 매매 단독 vs 개별** — 거부
- 한국 ETF 1115개 → 시총/거래대금 상위 150개 BT
- 발행사 다양성 검증: KODEX/TIGER/ARIRANG/KOSEF/HANARO/KBSTAR/SOL/PLUS/ACE 등
- 같은 인덱스 ETF도 발행사별 차이 큼 (예: 코스닥150 KODEX 1.21 vs TIGER 0.45)
- 단일 ETF 최강: KODEX 200TR Cal 1.36 (BT 측정)
- 신생 ETF (ACE AI반도체TOP3+ Cal 3.67) — 2023 상장 = OOS만 측정 = 비교 불공정
- 의미 있는 ETF (전체 7년): HANARO Fn K-반도체 Cal 2.02 (가장 강함)
- 개별 매매 baseline (Cal 3.06) 대비 모든 ETF 단독 매매 -55~80% 알파

**3. 개별 + ETF 블렌딩 (US 영감)** — 거부
- US 시스템에서 70:30 블렌딩이 안정성 보강 (MDD ↓)
- KR BT: 90:10 HANARO 반도체가 sweet spot — but **변화 미미** (Cal 동등, MDD -1.2%p만)
- 80:20+ 비중에서 MDD 오히려 ↑ (KR 알파 강해서 sl/ts 양쪽 누적)
- 섹터 ETF (반도체) 매칭 정당하지만 실효 효과 미미
- 사용자 결정: 효과 미미 → 채택 X (현행 개별 100% 유지)

**4. 메인+서브 슬롯 (Top 5~10 활용)** — 거부
- 사용자 안타까움: "Top 5~20도 상위 2~3% 좋은 종목인데 못 산다"
- EDA — rank별 forward return:
  - 20일: rank 1 +6.24% / rank 2 +8.30% / rank 3 +5.23% / rank 4-5 +2.97% / rank 8-10 +2.61%
  - 60일: rank 1 +20.5% / rank 2 +23.6% / rank 4+ +9~10% (감쇠)
- 알파는 wr Top 1~3에 집중. rank 4 이후 명확 감쇠
- BT 9 시나리오: 모든 메인+서브 (3+1, 3+2, 3+3, 3+4, 2+3, 균등 5/7슬롯) baseline 미달
  - 가장 가까운 (3 메인 30% + 1 서브 10%): Cal -20%, OOS -35%
  - MDD 분산 효과 거의 없음 (28~30% 유지)
- 사용자 직관과 데이터 불일치. 시스템은 Top 3 33% 균등이 최강
- **흥미로운 발견 (참고)**: rank 2가 rank 1보다 forward return ↑ (20일 8.30% vs 6.24%, 60일 23.6% vs 20.5%)

### 종합 — 5/27 변경 v80.20만 채택
- v80.20 (mom_10 + vol_low 가산) — 채택 + production 적용 완료
- 거래대금 임계, ETF 매매/블렌딩, 메인+서브 슬롯 — 모두 baseline 미달로 거부
- **현행 시스템 룰이 모든 후속 검증을 압도** — 추가 변경 없음
