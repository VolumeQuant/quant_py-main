...
```
Top 30 궤적에서 순위가 변한 종목에 "왜 변했는지" 태그 1개 자동 부착.
US 프로젝트(eps-momentum-us)와 동일한 태그 스타일 사용.

판단 로직: 순위 방향에 맞는 delta만 필터링 → 절대값 최대 팩터 선택
  순위 개선 (8→4): 양(+) delta 중 최대 → 💡저평가↑ / 💪실적↑ / 📈모멘텀↑
  순위 하락 (6→7): 음(-) delta 중 최대 → 📈주가↑ / ⚠️실적↓ / 📉모멘텀↓
  방향 맞는 delta 없으면 → 🔄상대변동

태그 7종:
  ⚠️실적↓   — Quality 스코어 하락 (가장 위험, 매도 검토)
  💪실적↑   — Quality 스코어 상승 (긍정적 펀더멘털)
  📈주가↑   — Value 하락 주도 (주가 상승으로 밸류에이션 반영, 보유 OK)
  💡저평가↑  — Value 상승 주도 (주가 하락으로 저평가 확대, 매수 기회)
  📈모멘텀↑  — Momentum 상승 주도 (추세 탑승)
  📉모멘텀↓  — Momentum 하락 주도 (추세 약화, 관망)
  🔄상대변동 — 개별 팩터 변화 미미, 다른 종목 움직임에 의한 상대적 변동

Threshold: V=0.05, Q=0.04, M=0.10 (팩터별 일평균 변동 기반)
비교 구간: ✅ 3일 검증 → T-0 vs T-2 (3일 누적), ⏳ 2일 검증 → T-0 vs T-1
표시: 순위 변화 있는 종목만 (1→1→1 등 변화 없으면 태그 없음)
```
...
*버전: 9.5 Slow In, Fast Out + 순위 태그 방향 일치 로직 | 최종 업데이트: 2026-02-21 | Generated by Claude Code*
