...
| **VIX 레이어** | CBOE VIX(FRED VIXCLS) → HY 보조 레이어, 5일 slope + Concordance Check |
...
│   └── credit_monitor.py         # 신용시장 모니터링 (US HY + 한국 BBB-)
│
├── [실행 스크립트] ─────────────────────────────────────────
...
### 4.8 신용시장 모니터링 (credit_monitor.py)
...
Layer 1 — 미국 HY Spread (FRED BAMLH0A0HYM2, API 키 불필요)
  Verdad 4분면 모델:
    ...
    Q1 회복기 (넓+하락) → 현금 0%, 적극 매수하세요
    Q2 성장기 (좁+하락) → 현금 20%, 평소대로 투자하세요
    Q3 과열기 (좁+상승) → 현금 20~30%, 신중하게~줄여가세요
    Q4 침체기 (넓+상승) → 현금 30~70%, 멈추고~확보하세요 (체류 기간별 단계적 확대)
...
### 4.9 VIX 변동성 레이어 (Layer 3 — credit_monitor.py)
...
공포완화 시: `💎 VIX(변동성) 36.2 ↓ — 공포완화 구간이에요. 현금 -10%`
...
*버전: 8.7 Slow In, Fast Out + Credit Monitor + VIX Layer 3 | 최종 업데이트: 2026-02-13 | Generated by Claude Code*
