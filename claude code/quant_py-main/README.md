...
| **VIX 레이어** | (도입 예정) CBOE VIX + VKOSPI → HY 보조 레이어, Concordance Check |
...
최종 현금비중 = Layer1 + Layer2 (최소 0%, 최대 70%)
종목당 비중 = 고정 20%
```

### 4.9 VIX 분석 (Layer 3 도입 예정)
...
**구현 우선순위**: ① FRED VIXCLS 수집 → ② VIX Layer 3 통합 → ③ 텔레그램 VIX 표시 → ④ VIX3M term structure → ⑤ 백테스트
...
*버전: 8.6 Slow In, Fast Out + Credit Monitor + VIX 분석 | 최종 업데이트: 2026-02-13 | Generated by Claude Code*
