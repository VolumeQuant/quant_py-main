...
```bash
# 1. 패키지 설치
pip install pykrx pandas numpy requests beautifulsoup4 lxml pyarrow tqdm aiohttp scipy html5lib
...
```
이익수익률 = EBIT / EV      (EV = 시가총액 + 총부채 - 현금)
투하자본수익률 = EBIT / IC   (IC = 자본 + 유동부채 - 현금 - 유동자산)
...
*버전: 3.5 | 최종 업데이트: 2026-02-07 | Generated by Claude Code*
