...
| **매수 추천** | 3거래일 연속 Top 30 교집합 (가중순위: T0×0.5+T1×0.3+T2×0.2), 최대 5종목 |
...
| **텔레그램** | 매일 06:30 KST 자동 전송 (4메시지: 개요 → 본문 → 생존 리스트 → AI 브리핑), HTML 볼드 |
...
```
메시지 1: 📊 퀀트 포트폴리오 — 활용 가이드
  - ▸ 종목 선정 과정 (①~④)
  - ▸ 매수·보유·매도 기준 (생애주기 가이드)
  - "매일 이 리포트를 확인하시면 됩니다"

메시지 2: 본문 (시장 + 매도 + 매수) — parse_mode='HTML'
  - 날짜 + 코스피/코스닥 각각 별도 줄 (개별 색상)
  - 이평선 경고 블록 (있을 때만 표시)
  - <b>⛔ 탈락 종목 — 매도 검토</b>: 어제 순위 → 오늘 순위 + 하락 사유 [V↓ Q↓ M↓]
  - <b>💎 매수 후보</b>: 3일 교집합 통과 종목 (최대 5개)
    ✅ 마커 + <b>종목명</b> + 한 줄 투자 근거 (실적 개선/저평가/고수익성 등)
    비중 20%, 가중순위, 가격, PER→F.PER/PBR/ROE, RSI, 52주위치
    3일순위: "1→1→2위" 형식
  - 0개면 "관망" 메시지

메시지 3: ✅ 생존 리스트 — 보유 유지 — parse_mode='HTML'
  - 종목명(순위) 형태로 나열 (쉼표 구분)
  - 없으면 '탈락 종목' 확인 유도

메시지 4: 🤖 AI 브리핑 (매수 후보 있을 때만)

콜드 스타트 (3일 미확보):
  - 채널 전송 스킵, 개인봇에만 전송
  - 3거래일 후 자동으로 채널 전송 시작
```
...
*버전: 5.2 Slow In, Fast Out | 최종 업데이트: 2026-02-10 | Generated by Claude Code*
