...
│   ├── send_telegram_auto.py        # ⭐ 자동화 텔레그램 전송 (GitHub Actions)
...
def filter_universe(market_cap_df, min_market_cap=1000, min_trading_value=30):
    """
    유니버스 필터링
...
[x] 날짜: 전일 거래일 자동 계산
[x] 포트폴리오: glob으로 최신 파일 탐색
[x] 기술지표: pykrx API 실시간
[x] 뉴스: Google News RSS 크롤링
[x] 전송: Telegram API

# ═══════════════════════════════════════════════════════════════
# 2단계 전략 시스템 (Value + Price)
# ═══════════════════════════════════════════════════════════════
...
RSI (40점): 낮을수록 좋음 (과매도 = 매수 기회)
  - ≤30: 40점, 31-50: 30점, 51-70: 20점
  - >70 + 신고가돌파: 20점 (감점 안 함)
  - >70 일반: 10점
...
필터링 (시세 뉴스 제외):
- "+X% 상승", "VI 발동", "상승폭 확대" 등 제외
- 실제 사업/실적 뉴스만 표시
- 종목명, 언론사명, [태그] 제거

표시 형식:
📰 주요뉴스: 마스크팩 인기에 1년 새 15배 뛴
📰⚠️ 삼성전자 HBM4 수율 격차 1.5배.. (부정적)
```
...
# GitHub Actions (매일 06:00 KST 자동 실행)
# - 채널: 공통종목 메시지 1개
# - 봇: 전체 메시지 3개
```
...
MIN_TRADING_VALUE = 30  # 최소 거래대금 (억원, 20일 평균)
```
...
*버전: 3.4 | 최종 업데이트: 2026-02-05 | Generated by Claude Code*
