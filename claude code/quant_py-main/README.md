...
| **텔레그램** | 매일 06:30 KST 자동 전송 (4메시지: 개요 → 본문 → 생존 리스트 → AI 브리핑) |
...
메시지 2: 본문 (시장 + 매도 + 매수)
  ...
  - 이평선 경고 블록 (있을 때만 표시)
  - ⛔ 탈락 종목 — 매도 검토: 어제 순위 → 오늘 순위 (유니버스 이탈 표시)
  - 💎 매수 후보: 3일 교집합 통과 종목
    종목별: 비중 15%, 가중순위, 가격, PER→F.PER/PBR/ROE, RSI, 52주위치
    3일순위: "1→1→2위" 형식
  - 0개면 "관망" 메시지
...
*버전: 5.1.1 Slow In, Fast Out | 최종 업데이트: 2026-02-10 | Generated by Claude Code*
