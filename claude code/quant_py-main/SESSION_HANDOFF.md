...
## 핵심 변경사항 (v17.3 — KRX 섹터 자동 분류 + Top 30 섹터 분포 통계)

### 2026-02-13 섹터 자동화: SECTOR_DB 수동 관리 → KRX 업종지수 자동 수집

**배경**: 기존 `SECTOR_DB`가 수동 딕셔너리(~103개)로 관리되어 Top 30의 절반이 '기타'로 표시됨. KRX 업종지수 구성종목을 pykrx로 자동 수집하여 2600+ 종목 커버.

| 항목 | Before | After |
|------|--------|-------|
| 섹터 소스 | `SECTOR_DB` 수동 딕셔너리 (~103개) | **KRX 업종지수 자동 수집** (2617개) |
| 커버리지 | ~50% (Top 30 중 절반 '기타') | **~97%** (거의 전 종목 커버) |
| 분류 체계 | 자유 문자열 | **KRX 세부업종 → 12개 대분류 매핑** |
| 섹터 저장 | 없음 | **ranking JSON에 `sector` 필드** |
| 텔레그램 표시 | AI 분석에 섹터만 전달 | **[1/3] 하단에 섹터 분포 통계 블록** |
| 집중 경고 | 없음 | **한 섹터 40%+ → ⚠️ 경고** |

**대분류 매핑** (KRX 업종 → 12개 통합):
```
전기전자 → 반도체/전자    운수장비 → 자동차
바이오/제약,의료정밀 → 바이오/제약   화학 → 화학/소재
전기가스,기계 → 에너지/유틸   유통,음식료,섬유/의류 → 소비재
건설 → 건설   서비스,통신,IT서비스 → 서비스/IT
금속,비금속,종이/목재 → 소재/금속   운수창고 → 물류
금융,증권,보험 → 금융   엔터/문화 → 엔터/문화   제조,기타제조 → 제조
```
...
─────────────────
📊 섹터 분포 (Top 30)
제조 9개(30%) · 소비재 5개(17%) · 서비스/IT 4개(13%) · ...
```

**수정 파일**:
1. `data_collector.py` — `get_krx_sector()` pykrx 업종지수 구성종목 방식으로 구현, 캐시 + fallback
2. `create_current_portfolio.py` — `KRX_SECTOR_MAP` 대분류 매핑, `get_broad_sector()`, ranking JSON에 sector 필드 주입
...
| AI 분석 | AI 브리핑 1개 (리스크만) | **2단계**: 🛡️ AI 리스크 필터 + 🎯 최종 추천 AI 멘트 |
