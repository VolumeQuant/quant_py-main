...
### 1. v68 데이터 축적 후 검증
- 2~4주 운영 후 score_100 분포 안정성 재확인
- 섹터 중립화 효과: 섹터 분산 개선 지속 여부
- 72/68 진입/퇴출 기준 적합성 재검증

### 2. FnGuide 안내문구 추가
- 한국 프로젝트: 분기 데이터 기반 → 장기 보유 스타일 안내
- 미국 프로젝트: 매일 EPS 변동 → 단기 회전 스타일 안내

### 3. FnGuide 재수집 정상 동작 모니터링
- `refresh_fnguide_cache.py` 수정 완료 (KRX 인증 + 병렬 2개)
- 다음 월요일 Windows 스케줄러 자동 실행 확인

### 4. 하락장 전환 시 검증
- M35 기여도가 현재 66% — 하락장에서 M 분포 좁아지면 V/Q 기여도 상승 예상
- 하락장 데이터 축적 시 가중치 재검토 (V30+Q35+M35 유지 vs V35+Q35+M30)
