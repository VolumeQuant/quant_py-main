...
### 1. entry/exit 그리드 서치 재검증
- scored ~300 데이터가 2~3주 축적된 후 실행
- `backtest_rank_grid.py` 사용
- 현재 TOP5/TOP15가 최적인지, TOP4나 TOP6이 나은지 검증
- 미국 프로젝트(TOP3/TOP15)와 비교 분석

### 2. FnGuide 안내문구 추가
- 한국 프로젝트: 분기 데이터 기반 → 장기 보유 스타일 안내
- 미국 프로젝트: 매일 EPS 변동 → 단기 회전 스타일 안내
- 각 프로젝트 텔레그램 메시지에 투자 스타일 안내 1줄 추가
...
### 4. V 가중치 재검토 (보류)
- V10/G35에서 V15/G30 변경 시 순위 변동 거의 없음 확인됨
- scored ~300 데이터 축적 후 가중치 그리드 서치(`backtest_weights.py`)로 재검증
- PER 극단 종목(189, 381 등) 처리 방안 추가 논의 필요
