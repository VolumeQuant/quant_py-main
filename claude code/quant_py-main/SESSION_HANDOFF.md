...
| 2 | `send_telegram_auto.py` | 4번째 메시지 "📊 맞춤형 ETF" 추가 — Top 10 종목 기준 테마 ETF 3개 조합 |

#### ETF 매칭 구조
```
Step 1 (Pro + Google Search): 종목별 ETF 검색 + "복수 종목 포함 ETF" 크로스 레퍼런스
Step 2 (Pro, 검색 OFF): Greedy 알고리즘 — Top 5 우선 커버, 중복 최소화
→ 3개 ETF 조합, 커버리지 6~7/10
```
