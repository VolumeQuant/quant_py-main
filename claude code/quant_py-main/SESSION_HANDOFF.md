...
## 핵심 변경사항 (v18.0 — ▸ 불릿 구분 + 3지표 Concordance 표시)

### 2026-02-13 시장 위험 지표 가독성 개선: 항목 불릿(▸) + Concordance 신호 일치도

**배경**: 🏦 신용시장 내 HY Spread와 한국 BBB-가 붙어있어 구분이 안 됨. 3가지 지표(HY, KR, VIX) 일치도를 결론에 반영.

| 항목 | Before (v17.9) | After (v18.0) |
|------|----------------|---------------|
| 항목 구분 | 텍스트만 나열 | **▸ 불릿 + 들여쓰기**로 항목 구분 |
| 결론 | `💰 투자 80%` 만 | **🟢🟢🟢 3/3 안정 — 확실한 신호** + `💰` |
| Concordance | 백엔드만 (HY vs VIX) | **3지표(HY+KR+VIX) 일치도 표시** |

**Concordance 판정 기준**:
- HY: Q1/Q2 → 🟢, Q3/Q4 → 🔴
- KR: 정상 → 🟢, 경계/위기 → 🔴
- VIX: stable → 🟢, warn → 🔴
- 3/3 안정 = 확실한 신호, 2/3 = 대체로 안정, 1/3 = 엇갈린 신호, 0/3 = 위험 신호

**수정 파일**:
1. `credit_monitor.py` — `format_credit_section()` ▸ 불릿 + Concordance 표시
2. `test_ui_preview.py` — 새 구조 반영
3. `README.md` / `SESSION_HANDOFF.md` — 문서 업데이트
