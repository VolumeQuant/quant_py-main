...
## 핵심 변경사항 (v18.5 — 모멘텀 개편 + AI 강화)

### 2026-02-20

**배경**: 순위 변동이 너무 적다는 피드백. 분석 결과: (1) Quality 25%가 FnGuide 주간 캐시라 매일 동일값 (2) Momentum `(12M-1M)/σ`가 최근 급등주를 벌주는 구조 (3) AI 분석이 인사말+뻔한 문구로 정보가 없음

| 항목 | Before | After |
|------|--------|-------|
| 가중치 | V45 + **Q25** + G10 + **M20** | V45 + **Q15** + G10 + **M30** |
| 모멘텀 공식 | `(12M - 1M) / σ` | `12M / σ` |
...
#### 왜 12M/σ인가
- `(12M-1M)/σ`는 Jegadeesh-Titman 단기 반전 효과 제거용이나, 보유기간 1~5일에만 유의미
- 이 전략은 "최소 2주 보유" → 모멘텀 지속 효과가 반전 효과 압도
- Value 45%가 밸류에이션 제어 → 추격매수 위험 이미 완화
- 제룡전기 사례: 최근 폭등인데 기존 공식은 오히려 벌줌

#### 변경 파일
- `strategy_b_multifactor.py`: 가중치 + 모멘텀 공식
