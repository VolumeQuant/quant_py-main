...
def send_telegram_message(buy, watch, latest_date):
    ...
    if buy:
        ...
        for r in buy[:5]:
            change_icon = "🔺" if r['daily_return'] > 0 else "🔻" if r['daily_return'] < 0 else "➖"
            msg += f"• *{r['name']}* {r['current_price']:,}원 {change_icon}{abs(r['daily_return']):.1f}%\n"
            msg += f"  PER:{r['per']:.1f} RSI:{r['rsi']:.0f} 점수:{r['entry_score']:.2f}⭐\n"
        if len(buy) > 5:
            msg += f"  ... 외 {len(buy) - 5}개\n"
    else:
        msg += "🟢 매수 적기: 없음\n"
    ...
    if watch:
        ...
        top3 = [r['name'] for r in watch[:3]]
        msg += f"  {', '.join(top3)}" + (" ..." if len(watch) > 3 else "") + "\n"

    msg += "\n_Generated by Quant Bot_"
