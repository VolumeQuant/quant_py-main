...
def format_credit_section(credit: dict) -> str:
    ...
    if hy:
        lines.append(f"{hy['quadrant_icon']} <b>신용시장 — {hy['quadrant_label']}</b>")
        lines.append(f"📊 미국 HY: {hy['hy_spread']:.2f}% (중위 {hy['median_10y']:.2f}%)")

        if kr:
            lines.append(f"📊 한국 BBB-: {kr['spread']:.1f}%p {kr['regime_icon']} {kr['regime_label']}")

        # 현금비중 + 액션
        if final_cash == 0:
            cash_line = f"📊 투자 100% · {final_action}"
        else:
            cash_line = f"📊 현금 {final_cash}% · {final_action}"
            if hy['quadrant'] in ('Q3', 'Q4'):
                cash_line += f" ({hy['quadrant']} {hy['q_days']}일째)"
        ...
        lines.append(f'📊 기본 현금 {final_cash}% · {final_action}')
