...
### 문제 1: OHLCV 증분 업데이트 버그
- 하루치 가격 추가할 때 `market='ALL'`로 전 시장 2800종목을 캐시에 넣음
- 캐시가 오염되면 MA120 계산 대상이 뻥튀기 → scored 증가
- **해결 완료** (커밋 84ab431): 유니버스 종목만 필터링하도록 수정

### 문제 2: KRX 인증 고장 (2주간)
- 비밀번호 변경 후 미반영 → pykrx가 데이터를 못 가져옴
- OHLCV 증분 업데이트 실패 → 캐시가 오래된 가격에 멈춤
- **해결 완료**: 비밀번호 수정 + 중복 로그인 처리 (krx_auth.py)

### 문제 3: 시장 폭등 시 유니버스 확장 (미해결)
- 3/18 코스피 +5.04% → MA120 통과 종목 246→562개로 급증 가능
- scored 139→318, 삼성전자 4위→7위
- 이건 버그가 아니라 시장 현실. 72/68 기준이 흔들릴 수 있음
- **홈PC에서 3/18 가격 반영 후 확인 필요**

### 현재 매일 유니버스 계산 구조
- 시총/거래대금: 매일 pykrx에서 실시간 조회 ✅
- OHLCV(가격): 매일 증분 업데이트 (버그 수정 완료) ✅
- FnGuide(재무제표): 분기 데이터라 주 1회 갱신이면 충분 ✅
- 증분 버그 + KRX 인증이 고쳐졌으니 앞으로는 매일 최신 가격으로 정확하게 계산됨
