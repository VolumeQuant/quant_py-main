...
def compute_risk_flags(s):
    ...
    원칙: 4겹 검증(유니버스→멀티팩터→MA60→3일교집합)을 통과한 종목이므로
    모델이 이미 평가한 밸류에이션/가격 수준은 다시 벌점 주지 않는다.
    리스크 필터는 "당일 갑자기 발생한 이벤트"만 잡는다.

    제거된 플래그 (모델과 충돌):
    - 52주 급락: "싸게 사자" 철학과 정면 충돌 + MA60이 가치함정 차단
    ...
    # 2. 전일 급락 (daily_chg <= -5%) — 악재 확인 필요
    if chg <= -5:
        flags.append(f"⚠️ 전일 {chg:.1f}% 급락")

    # 3. 전일 급등 (daily_chg >= +8%) — 추격매수 위험
    if chg >= 8:
        flags.append(f"🔺 전일 +{chg:.1f}% 급등 (추격매수 주의)")

    # 4. 거래량 폭발 (vol_ratio >= 3.0) — 비정상 움직임
    if vol >= 3.0:
