...
def main():
    ...
    # 기대성장 = 선행EPS / TTM실적EPS (커버 유니버스 전체)
    grow = {}
    for t in targets:
        fe = fwd_eps.get(t); te = ttm_eps(t)
        if fe and te and te > 0: grow[t] = fe / te
    # cross-sec 상위100 = 확인셋
    ranked = sorted(grow.items(), key=lambda z: -z[1])
    confirm = set(t for t, _ in ranked[:TOPN_CONFIRM])
    cutoff = ranked[min(TOPN_CONFIRM, len(ranked)) - 1][1] if ranked else None
    print(f"\n=== 융합 forward 추적기 (cross-sec 상위{TOPN_CONFIRM}, ×{CW:.0f}) ===")
    print(f"production {pbd} / 커버 {len(covered)} / 기대성장계산 {len(grow)} / 상위{TOPN_CONFIRM} 컷 {('%.2fx'%cutoff) if cutoff else 'NA'}\n")
    # 보유 top3 확인 + 가중치
    held_info, raw_w = [], []
